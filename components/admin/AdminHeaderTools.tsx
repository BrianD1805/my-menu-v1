...
function trialStatusText(trial?: TenantTrialState | null) {
  ...
  if (trial.trialDaysRemaining === null)
    return "Trial active — choose a plan when ready";
...
function trialTone(trial?: TenantTrialState | null) {
  ...
  if ((trial.trialDaysRemaining ?? 99) <= 2)
    return "border-[#FFB168]/55 bg-[#FFF7F0] text-[#C84F2A]";
  return "border-emerald-200 bg-emerald-50 text-emerald-900";
}

export default function AdminHeaderTools({
  tenantSlug,
  trialState,
  mode = "both",
}: {
  ...
  useEffect(() => {
    setMounted(true);
    if (mode !== "billing") loadChecklistSummary();
    const refresh = () => {
      setNow(Date.now());
      if (mode !== "billing") loadChecklistSummary();
    };
      ...
      <div className="flex shrink-0 items-stretch gap-1.5 sm:gap-2 sm:items-center sm:justify-end">
        {showChecklistButton ? (
          ...
          >
            ...
            <span className="mt-1 text-[10px] font-black uppercase tracking-[0.15em] text-[#5C5F66] sm:text-[11px]">
              Checklist
            </span>
          </button>
        ) : null}
        ...
        {showBillingButton ? (
          <button
            type="button"
            onClick={() => {
              const shouldOpenActivation = Boolean(
                trialState?.isTrialExpired &&
                trialState?.subscriptionStatus !== "active" &&
                trialState?.trialStatus !== "converted",
              );
          ...
          >
            <span className="text-sm font-black leading-none sm:text-base">
              {trialShortLabel(trialState)}
            </span>
            <span className="mt-1 text-[10px] font-black uppercase tracking-[0.15em] opacity-75 sm:text-[11px]">
              {trialSecondaryLabel(trialState)}
            </span>
          </button>
        ) : null}
      </div>
                  ...
                  <div className="min-h-0 flex-1 overflow-y-auto overscroll-contain px-5 py-5 sm:px-7 sm:py-6">
                    {modal === "checklist" ? (
                      ...
                      <div className="rounded-[26px] border border-emerald-900/10 bg-white p-5 text-[#1F2328] sm:p-6">
                        ...
                        {trialState?.subscriptionStatus === "active" ||
                        trialState?.trialStatus === "converted" ? null : (
