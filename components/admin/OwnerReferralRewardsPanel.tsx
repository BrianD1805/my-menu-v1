...
export default function OwnerReferralRewardsPanel() {
  ...
  const cards = [
    ...
    { key: "active" as const, label: "Monthly liability", value: moneyByCurrency(summary.totalsByCurrency, "estimatedMonthlyLiability", summary.estimatedMonthlyLiability), hint: "Estimated active rewards", className: "border-[#8FB6D9]/35 bg-[#8FB6D9]/10 text-[#BFD8EE]" },
    { key: "pending" as const, label: "Pending credits", value: moneyByCurrency(summary.totalsByCurrency, "pendingCredits", summary.totalPendingCredits), hint: "Credited but unpaid", className: "border-[#8FB6D9]/35 bg-[#336699]/10 text-[#EAF3FB]" },
    ...
    { key: "trial" as const, label: "Trial", value: summary.trialRewards, hint: "Not paying yet", className: "border-[#8FB6D9]/35 bg-[#8FB6D9]/10 text-[#BFD8EE]" },
    ...
    <section className="overflow-hidden rounded-[30px] border border-[#0E0E10]/10 bg-white shadow-[0_18px_50px_rgba(14,14,16,0.08)]">
      <div className="bg-gradient-to-br from-[#0E0E10] via-[#1B1B1F] to-[#332019] p-5 text-white sm:p-6">
        <div className="flex flex-col gap-4 lg:flex-row lg:items-start lg:justify-between">
          <div>
            <p className="text-xs font-semibold uppercase tracking-[0.22em] text-[#8FB6D9]">Referral rewards</p>
            <h2 className="mt-2 text-2xl font-semibold tracking-tight sm:text-3xl">Monthly tenant credits</h2>
          ...
          <button type="button" onClick={() => void loadReferrals()} disabled={loading || !canLoad} className="inline-flex min-h-11 items-center justify-center rounded-2xl bg-white px-5 py-3 text-sm font-semibold text-[#0E0E10] transition hover:bg-[#F3F7FA] disabled:cursor-not-allowed disabled:opacity-60">
        ...
        <div className="mt-5 grid gap-3 sm:grid-cols-2 xl:grid-cols-3">
          {cards.map((card) => {
            ...
            return (
              <button key={`${card.label}-${card.key}`} type="button" onClick={() => setFilter(card.key)} className={["rounded-[22px] border p-4 text-left transition hover:-translate-y-0.5 hover:bg-white/15 focus:outline-none focus-visible:ring-2 focus-visible:ring-[#8FB6D9]", card.className, selected ? "ring-2 ring-[#8FB6D9] ring-offset-2 ring-offset-[#0E0E10]" : ""].join(" ")}>
                <p className="text-[11px] font-semibold uppercase tracking-[0.16em] opacity-75">{card.label}</p>
      ...
      <div className="border-t border-[#0E0E10]/10 p-5 sm:p-6">
        ...
        <div className="mb-4 rounded-[24px] border border-[#0E0E10]/8 bg-[#F3F7FA] p-4">
          <p className="text-xs font-semibold uppercase tracking-[0.18em] text-[#28547D]">Selected view</p>
          <h3 className="mt-1 text-xl font-semibold text-[#0E0E10]">{filterTitle(filter)}</h3>
          <p className="mt-1 text-sm font-semibold text-[#68707A]">
            Showing <span className="font-semibold text-[#0E0E10]">{filteredRows.length}</span> referral records across <span className="font-semibold text-[#0E0E10]">{referralGroups.length}</span> referring tenants.
          </p>
          <p className="mt-1 text-sm font-semibold text-[#68707A]">
            Estimated active monthly reward liability: <span className="font-semibold text-[#0E0E10]">{moneyByCurrency(summary.totalsByCurrency, "estimatedMonthlyLiability", summary.estimatedMonthlyLiability)}</span> · Recorded subscription payments: <span className="font-semibold text-[#0E0E10]">{moneyByCurrency(summary.totalsByCurrency, "subscriptionPayments", summary.totalSubscriptionPayments || 0)}</span> · Pending credits: <span className="font-semibold text-[#0E0E10]">{moneyByCurrency(summary.totalsByCurrency, "pendingCredits", summary.totalPendingCredits)}</span> · Paid credits: <span className="font-semibold text-[#0E0E10]">{moneyByCurrency(summary.totalsByCurrency, "paidCredits", summary.totalPaidCredits)}</span>
          </p>
        </div>

        {!loading && rows.length === 0 ? <div className="rounded-[24px] border border-dashed border-[#0E0E10]/18 bg-[#F3F7FA] p-5 text-sm leading-6 text-[#5C5F66]">No referral signups have been captured yet.</div> : null}
              ...
              <article key={row.signup.id} className="rounded-[26px] border border-[#0E0E10]/10 bg-white p-4">
                <div className="flex flex-col gap-3 lg:flex-row lg:items-start lg:justify-between">
                  <div>
                    <div className="flex flex-wrap items-center gap-2">
                      <h3 className="text-xl font-semibold text-[#0E0E10]">{referrerName}</h3>
                      <span className={["rounded-full px-3 py-1 text-[11px] font-semibold uppercase tracking-[0.14em] ring-1", statusClasses(row.reward?.reward_status)].join(" ")}>{row.reward?.reward_status || "trial"}</span>
                      <span className="rounded-full bg-white px-3 py-1 text-[11px] font-semibold uppercase tracking-[0.14em] text-[#5C5F66] ring-1 ring-[#0E0E10]/10">{row.reward?.reward_rate_percent ?? row.source?.reward_rate_percent ?? 15}% monthly</span>
                    </div>
                    <p className="mt-1 text-sm font-semibold uppercase tracking-[0.14em] text-[#28547D]">{isAffiliateReferral ? "Approved affiliate" : "Referring tenant"}</p>
                    <p className="mt-2 text-sm font-bold text-[#0E0E10]">Referred store: <span className="text-[#28547D]">{referredName}</span></p>
                    ...
                    {isAffiliateReferral && row.reward?.secondary_reward_rate_percent ? (
                      <p className="mt-2 rounded-2xl border border-[#336699]/20 bg-[#F3F7FA] px-3 py-2 text-xs font-bold text-[#28547D]">
                  ...
                  <div className="rounded-2xl border border-[#0E0E10]/10 bg-white px-4 py-3 text-sm text-right">
                    <p className="text-xs font-semibold uppercase tracking-[0.16em] text-[#68707A]">Estimated monthly credit</p>
                    <p className="mt-1 text-2xl font-semibold text-[#0E0E10]">{money(row.reward?.estimated_monthly_reward, currency)}</p>
                    {isAffiliateReferral && row.reward?.secondary_referrer_tenant_id ? (
                      <p className="mt-1 text-xs font-semibold text-[#28547D]">Tenant intro estimate {money(row.reward?.secondary_estimated_monthly_reward || 0, currency)}</p>
                    ) : null}
                    ...
                    {isAffiliateReferral && (row.totals.secondaryPendingAmount || row.totals.secondaryPaidAmount) ? (
                      <p className="mt-1 text-xs font-bold text-[#28547D]">Tenant share pending {money(row.totals.secondaryPendingAmount || 0, currency)} · paid {money(row.totals.secondaryPaidAmount || 0, currency)}</p>
                    ) : null}
                  </div>
                </div>
                ...
                {draft && row.reward ? (
                  <div className="mt-4 grid gap-3 rounded-[22px] border border-[#0E0E10]/8 bg-white p-4 lg:grid-cols-7">
                    <label className="block lg:col-span-1"><span className="mb-1 block text-xs font-semibold text-[#0E0E10]">Affiliate %</span><input value={draft.rewardRatePercent} onChange={(event) => updateDraft(row.reward!.id, "rewardRatePercent", event.target.value)} inputMode="decimal" className="min-h-11 w-full rounded-2xl border border-[#0E0E10]/12 px-3 py-2 text-sm font-bold outline-none focus:border-[#336699]" /></label>
                    {isAffiliateReferral && row.reward.secondary_referrer_tenant_id ? (
                      <label className="block lg:col-span-1"><span className="mb-1 block text-xs font-semibold text-[#0E0E10]">Tenant intro %</span><input value={draft.tenantIntroductionSharePercent} onChange={(event) => updateDraft(row.reward!.id, "tenantIntroductionSharePercent", event.target.value)} inputMode="decimal" placeholder="5" className="min-h-11 w-full rounded-2xl border border-[#0E0E10]/12 px-3 py-2 text-sm font-bold outline-none focus:border-[#336699]" /></label>
                    ) : null}
                    <label className="block lg:col-span-1"><span className="mb-1 block text-xs font-semibold text-[#0E0E10]">Monthly fee</span><input value={draft.monthlySubscriptionAmount} onChange={(event) => updateDraft(row.reward!.id, "monthlySubscriptionAmount", event.target.value)} inputMode="decimal" placeholder="29.00" className="min-h-11 w-full rounded-2xl border border-[#0E0E10]/12 px-3 py-2 text-sm font-bold outline-none focus:border-[#336699]" /></label>
                    <label className="block lg:col-span-1"><span className="mb-1 block text-xs font-semibold text-[#0E0E10]">Currency</span><input value={draft.currencyCode} onChange={(event) => updateDraft(row.reward!.id, "currencyCode", event.target.value.toUpperCase().slice(0, 3))} className="min-h-11 w-full rounded-2xl border border-[#0E0E10]/12 px-3 py-2 text-sm font-bold uppercase outline-none focus:border-[#336699]" /></label>
                    <label className="block lg:col-span-1"><span className="mb-1 block text-xs font-semibold text-[#0E0E10]">Status</span><select value={draft.rewardStatus} onChange={(event) => updateDraft(row.reward!.id, "rewardStatus", event.target.value)} className="min-h-11 w-full rounded-2xl border border-[#0E0E10]/12 bg-white px-3 py-2 text-sm font-bold outline-none focus:border-[#336699]"><option value="trial">Trial</option><option value="active">Active</option><option value="paused">Paused</option><option value="cancelled">Cancelled</option></select></label>
                    <label className="block lg:col-span-2"><span className="mb-1 block text-xs font-semibold text-[#0E0E10]">Notes</span><input value={draft.notes} onChange={(event) => updateDraft(row.reward!.id, "notes", event.target.value)} placeholder="Internal note" className="min-h-11 w-full rounded-2xl border border-[#0E0E10]/12 px-3 py-2 text-sm font-bold outline-none focus:border-[#336699]" /></label>
                    <div className="flex flex-col gap-2 lg:col-span-7 sm:flex-row sm:flex-wrap">
                      <button type="button" onClick={() => void saveReward(row)} disabled={busyId === row.reward.id} className="inline-flex min-h-10 items-center justify-center rounded-2xl bg-[#0E0E10] px-4 py-2 text-xs font-semibold text-white transition hover:bg-[#252528] disabled:cursor-not-allowed disabled:opacity-60">{busyId === row.reward.id ? "Saving..." : "Save reward settings"}</button>
                      <label className="inline-flex min-h-10 items-center gap-2 rounded-2xl border border-[#0E0E10]/10 bg-[#F3F7FA] px-3 py-2 text-xs font-semibold text-[#0E0E10]">Paid month <input type="month" value={draft.creditMonth} onChange={(event) => updateDraft(row.reward!.id, "creditMonth", event.target.value)} className="bg-transparent font-semibold outline-none" /></label>
                      <input value={draft.paymentReference} onChange={(event) => updateDraft(row.reward!.id, "paymentReference", event.target.value)} placeholder="Payment reference / note" className="min-h-10 rounded-2xl border border-[#0E0E10]/10 bg-white px-3 py-2 text-xs font-bold outline-none focus:border-[#336699]" />
                      <button type="button" onClick={() => void recordSubscriptionPayment(row)} disabled={busyId === `${row.reward.id}:payment`} className="inline-flex min-h-10 items-center justify-center rounded-2xl border border-[#336699]/25 bg-[#F3F7FA] px-4 py-2 text-xs font-semibold text-[#28547D] transition hover:bg-white disabled:cursor-not-allowed disabled:opacity-60">{busyId === `${row.reward.id}:payment` ? "Recording..." : "Record subscription payment"}</button>
                    </div>
                  </div>
                ) : <p className="mt-4 rounded-2xl border border-[#8FB6D9] bg-[#EAF3FB] px-4 py-3 text-sm font-bold text-[#28547D]">Reward row is being prepared. Refresh this page in a moment.</p>}
                      ...
                      {row.credits.slice(0, 8).map((credit) => (
                        <div key={credit.id} className="flex flex-col gap-2 rounded-2xl border border-[#0E0E10]/8 bg-white px-3 py-3 text-sm sm:flex-row sm:items-center sm:justify-between">
                          <div>
                            ...
                            {credit.secondary_reward_amount ? <p className="text-xs font-bold text-[#28547D]">Tenant introduction share: {money(credit.secondary_reward_amount, credit.currency_code || currency)} at {credit.secondary_reward_rate_percent || 5}%</p> : null}
                          </div>
                          <div className="flex flex-wrap items-center gap-2">
                            <span className={["rounded-full px-3 py-1 text-[11px] font-semibold uppercase tracking-[0.14em] ring-1", creditClasses(credit.credit_status)].join(" ")}>{credit.credit_status || "pending"}</span>
                            {credit.credit_status !== "paid" ? <button type="button" onClick={() => void markCredit(credit.id, "paid")} disabled={busyId === credit.id} className="rounded-xl bg-[#0E0E10] px-3 py-2 text-xs font-semibold text-white">Mark paid</button> : null}
                            {credit.credit_status !== "void" ? <button type="button" onClick={() => void markCredit(credit.id, "void")} disabled={busyId === credit.id} className="rounded-xl border border-[#0E0E10]/10 bg-white px-3 py-2 text-xs font-semibold text-[#0E0E10]">Void</button> : null}
                          </div>
                        </div>
                      ))}
                    </div>
