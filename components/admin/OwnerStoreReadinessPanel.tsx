...
type StoreReadiness = { id: string; name: string; slug: string; status: string; createdAt: string | null; storeAddress: string; storefrontUrl: string; adminLoginUrl: string; readiness: { score: number; label: string; tone: string; readyCount: number; totalChecks: number; blockingIssues: number }; counts: { categories: number; products: number; activeProducts: number; productPhotos: number; adminPushDevices: number; orders: number; emailSent: number; emailFailed: number }; checks: ReadinessCheck[] };
type Payload = { stores: StoreReadiness[]; summary: { totalStores: number; readyStores: number; nearlyReadyStores: number; needsSetupStores: number; missingProducts: number; missingAdminPush: number } };
...
export default function OwnerStoreReadinessPanel() {
  ...
  const summary = payload?.summary || { totalStores: 0, readyStores: 0, nearlyReadyStores: 0, needsSetupStores: 0, missingProducts: 0, missingAdminPush: 0 };
