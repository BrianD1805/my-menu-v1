...
type FormState = {
  ...
  privacyPolicyShowOnStorefront: boolean;
  termsOfServiceTitle: string;
  termsOfServiceBody: string;
  termsOfServiceShowOnStorefront: boolean;
  footerBlurb: string;
  footerNotice: string;
  ...
  invoicePaymentsInvoiceEnabled: boolean;
  invoicePaymentsDepositEnabled: boolean;
  invoicePaymentsBalanceEnabled: boolean;
};
...
const THEME_PRESETS: ThemePreset[] = [
  makePreset(
    "Premium Blue & Orange",
    "Clean, professional and tech-led.",
    "#336699",
    "#F28C28",
    "#F3F8FC",
    "#BED3E8",
  ...
  makePreset(
    "Forest Green & Gold",
    "Warm, natural and restaurant-friendly.",
    "#1F5C3B",
    "#D8A63A",
    "#F4F7EF",
    "#C9D8B8",
...
const THEME_GROUPS: Array<{
  ...
  options?: Array<{
    key: "favouritesCardShadowEnabled";
    label: string;
    help: string;
  }>;
}> = [
  ...
  },
  {
    ...
    fields: [
      ...
      {
        key: "welcomeActionIconBackground",
...

  {
    ...
    fields: [
      ...
      { key: "rewardsSilverHeaderText", label: "Silver header text" },
      { key: "rewardsSilverLabelText", label: "Silver label text" },
      { key: "rewardsSilverCurrentPanelBackground", label: "Silver current panel background" },
      { key: "rewardsSilverCurrentPanelBorder", label: "Silver current panel border" },
      { key: "rewardsSilverCurrentPillBackground", label: "Silver current pill background" },
      { key: "rewardsSilverCurrentPillText", label: "Silver current pill text" },
      { key: "rewardsSilverProgressFill", label: "Silver progress fill" },
      { key: "rewardsGoldTopEdge", label: "Gold top edge" },
      { key: "rewardsGoldHeaderBackground", label: "Gold header background" },
      { key: "rewardsGoldHeaderBlend", label: "Gold header blend" },
      { key: "rewardsGoldHeaderText", label: "Gold header text" },
      { key: "rewardsGoldLabelText", label: "Gold label text" },
      { key: "rewardsGoldCurrentPanelBackground", label: "Gold current panel background" },
      { key: "rewardsGoldCurrentPanelBorder", label: "Gold current panel border" },
      { key: "rewardsGoldCurrentPillBackground", label: "Gold current pill background" },
      { key: "rewardsGoldCurrentPillText", label: "Gold current pill text" },
      { key: "rewardsGoldProgressFill", label: "Gold progress fill" },
      { key: "rewardsPlatinumTopEdge", label: "Platinum top edge" },
      { key: "rewardsPlatinumHeaderBackground", label: "Platinum header background" },
      { key: "rewardsPlatinumHeaderBlend", label: "Platinum header blend" },
      { key: "rewardsPlatinumHeaderText", label: "Platinum header text" },
      { key: "rewardsPlatinumLabelText", label: "Platinum label text" },
      { key: "rewardsPlatinumCurrentPanelBackground", label: "Platinum current panel background" },
      { key: "rewardsPlatinumCurrentPanelBorder", label: "Platinum current panel border" },
      { key: "rewardsPlatinumCurrentPillBackground", label: "Platinum current pill background" },
      { key: "rewardsPlatinumCurrentPillText", label: "Platinum current pill text" },
      { key: "rewardsPlatinumProgressFill", label: "Platinum progress fill" },
    ],
  },
  {
    id: "offers",
  ...
  },
  {
    ...
    title: "Product Cards",
    description:
      "Product card surfaces, titles, price box and Add/More buttons.",
    fields: [
      ...
      {
        key: "productHeartUntickedBackground",
        label: "Heart unticked background",
      },
    ...
    options: [
      {
        key: "favouritesCardShadowEnabled",
        label: "Card shadow",
        help: "Turn the favourite card shadow on or off.",
      },
    ],
  },
  {
...
async function extractLogoColours(logoUrl: string): Promise<string[]> {
  ...
  if (!response.ok)
    throw new Error(
      "Could not read the uploaded logo. Please refresh and try again.",
    );
  ...
  try {
    const image = await new Promise<HTMLImageElement>((resolve, reject) => {
      ...
      img.onerror = () =>
        reject(new Error("Could not analyse this logo image."));
      img.src = objectUrl;
    });
    ...
    const ratio = Math.min(
      maxSize / Math.max(image.naturalWidth || 1, image.naturalHeight || 1),
      1,
    );
    canvas.width = Math.max(
      1,
      Math.round((image.naturalWidth || maxSize) * ratio),
    );
    canvas.height = Math.max(
      1,
      Math.round((image.naturalHeight || maxSize) * ratio),
    );
    ...
    for (let i = 0; i < pixels.length; i += 8) {
      ...
      const key = rgbToHex(
        ...
        Math.round(b / quantiseBy) * quantiseBy,
      );
    ...
    const ranked = Array.from(buckets.entries())
      .map(([colour, data]) => ({
        colour,
        ...data,
        hueFamily: Math.floor(data.hue / 30),
      }))
      .sort(
        (a, b) =>
          b.score - a.score || b.saturation - a.saturation || b.count - a.count,
      );
...
export default function TenantSettingsForm({
  ...
  const initialForm = useMemo(
    () => ({
      ...initial,
      storefrontTheme: normaliseTheme(initial.storefrontTheme, initial),
    }),
    [initial],
  );
  ...
  const [logoPalettePreset, setLogoPalettePreset] =
    useState<ThemePreset | null>(
      initialForm.storefrontTheme?.selectedPreset === LOGO_PALETTE_PRESET_NAME
        ? buildLogoPalettePreset(
            initialForm.storefrontTheme.logoPaletteColours?.length
              ? initialForm.storefrontTheme.logoPaletteColours
              : [
                  initialForm.primaryColor,
                  initialForm.accentColor,
                  initialForm.backgroundTint,
                  initialForm.borderColor,
                  initialForm.textColor,
                ],
          )
        : null,
    );
  ...
  const [mpesaDiagnosticAction, setMpesaDiagnosticAction] = useState<
    "check" | "create_order" | "mark_failed" | null
  >(null);
  const [mpesaDiagnosticResult, setMpesaDiagnosticResult] =
    useState<MpesaDiagnosticsResult | null>(null);
  ...
  const previewSubheading =
    form.storefrontSubheading.trim() ||
    "Tap into the details for more information, or add favourites straight to your order.";
  const footerBlurb =
    form.footerBlurb.trim() || "Thank you for ordering with us.";
  const footerNotice =
  ...
  const moneySettings = {
    ...
    currencyCode:
      form.currencyCode.trim() || DEFAULT_MONEY_SETTINGS.currencyCode,
    currencySymbol:
      form.currencySymbol.trim() || DEFAULT_MONEY_SETTINGS.currencySymbol,
  ...
  const formValueChanged = (keys: Array<keyof FormState>) =>
    keys.some(
      (key) => JSON.stringify(form[key]) !== JSON.stringify(savedForm[key]),
    );
  const brandingDirty = formValueChanged([
    "businessDisplayName",
    "adminHeadingLabel",
    "storefrontHeading",
  ...
  const contactDirty = formValueChanged([
    "contactPhone",
    "contactWhatsApp",
    "contactEmail",
    "contactAddress",
  ...
  const rewardsDirty = formValueChanged([
    "rewardsEnabled",
    "rewardsProgramName",
  ...
  const discountsDirty = formValueChanged([
    "discountsEnabled",
  ...
  const receiptInfoDirty = formValueChanged([
    "receiptDocumentName",
    "receiptTaxLabel",
    "receiptTaxNumber",
    "receiptTaxRatePercent",
    "receiptExtraField1Enabled",
    "receiptExtraField1Label",
    "receiptExtraField1Value",
    "receiptExtraField2Enabled",
    "receiptExtraField2Label",
    "receiptExtraField2Value",
    "receiptFooterMessage",
    "receiptBrandImageMode",
  ]);
  const seoDirty = formValueChanged([
    "seoPageName",
    "seoMetaDescription",
    "seoKeywords",
    "seoCanonicalUrl",
  ...
  const invoicePaymentsDirty = formValueChanged([
  ...
  const paymentDirty = formValueChanged([
    "enableCashOnCollection",
    ...
    "stripeCustomerTestMode",
    "stripeCustomerSetupNotes",
  ...
  const stripeCredentialReady = Boolean(
    form.stripeCustomerPublishableKey.trim() &&
    (form.stripeCustomerSecretKeySet ||
      form.stripeCustomerSecretKeyInput.trim()) &&
    (form.stripeCustomerWebhookSecretSet ||
      form.stripeCustomerWebhookSecretInput.trim()),
  );
  const yocoCurrencyAllowed =
    String(form.currencyCode || "")
      .trim()
      .toUpperCase() === "ZAR";
  const yocoCredentialReady = Boolean(
    form.yocoCustomerSecretKeySet || form.yocoCustomerSecretKeyInput.trim(),
  );
  const yocoWebhookReady = Boolean(
    form.yocoCustomerWebhookSecretSet ||
    form.yocoCustomerWebhookSecretInput.trim(),
  );
  ...
  const yocoReadyForCheckout = Boolean(
  ...
  const mpesaCurrencyAllowed =
    String(form.currencyCode || "")
      .trim()
      .toUpperCase() === "KES";
  const mpesaCredentialReady = Boolean(
  ...
  const darajaCurrencyAllowed =
    String(form.currencyCode || "")
      .trim()
      .toUpperCase() === "KES";
  const darajaCredentialReady = Boolean(
    form.darajaConsumerKey.trim() &&
    (form.darajaConsumerSecretSet || form.darajaConsumerSecretInput.trim()) &&
    form.darajaShortcode.trim() &&
    (form.darajaPasskeySet || form.darajaPasskeyInput.trim()),
  );
  const darajaModeLive = form.darajaCustomerMode === "live";
  const darajaUsesSandboxShortcode = form.darajaShortcode.trim() === "174379";
  const darajaLiveReadinessOk = Boolean(
    darajaCurrencyAllowed &&
    darajaCredentialReady &&
    (!darajaModeLive || !darajaUsesSandboxShortcode),
  );
  const mpesaReadyForCheckout = Boolean(
    form.enableMpesaCustomerPayments &&
    form.mpesaCustomerPaymentsLive &&
  ...
  const themeGroupDirty = (group: (typeof THEME_GROUPS)[number]) =>
    group.fields.some(
      (field) =>
        String(theme[field.key] || "") !== String(savedTheme[field.key] || ""),
    ) ||
    Boolean(
      group.options?.some(
        (option) =>
          Boolean(theme[option.key]) !== Boolean(savedTheme[option.key]),
      ),
    );
  ...
  const availableThemePresets = logoPalettePreset
    ? [...THEME_PRESETS, logoPalettePreset]
    : THEME_PRESETS;
  const activePreset = availableThemePresets.find(
    (preset) => theme.selectedPreset === preset.name,
  );
  ...
  function addDiscountRule() {
    ...
    setForm((current) => ({
      ...current,
      discountRules: [...(current.discountRules || []), next],
    }));
  }
  ...
  function toggleDiscountProduct(
    index: number,
    productId: string,
    checked: boolean,
  ) {
    ...
    const next = checked
      ? Array.from(new Set([...current, productId])).slice(0, 3)
  ...
  function updateStorefrontThemeSetting(
  ...
  async function generateLogoPalette() {
    ...
    try {
      ...
      if (colours.length < 2)
        throw new Error("Could not find enough usable colours in this logo.");
      ...
      setMessage(
        error instanceof Error
          ? error.message
          : "Could not generate a palette from this logo.",
      );
    } finally {
  ...
  function scrollSettingsToTop() {
    settingsTopRef.current?.scrollIntoView({
      behavior: "smooth",
      block: "start",
    });
  }
  ...
  async function uploadAsset(file: File, kind: "logo" | "favicon") {
    ...
    try {
      ...
      const payload = await response.json();
      if (!response.ok)
        throw new Error(payload.error || `Failed to upload ${kind}`);
      ...
      const successMessage =
        payload.message ||
        `${kind === "logo" ? "Logo" : "Favicon"} uploaded and saved.`;
      setTone("success");
      setMessage(successMessage);
      showToast(successMessage, "success");
    } catch (error) {
    ...
    try {
      ...
      if (adminWorkspaceDirty) {
        const checklistResponse = await fetch("/api/admin/launch-checklist", {
          ...
          headers: { "Content-Type": "application/json" },
          body: JSON.stringify({
            checklistKey: "__dismissed",
            status: form.showAdminLaunchChecklist ? "pending" : "complete",
          }),
        });
        const checklistPayload = await checklistResponse
          .json()
          .catch(() => ({}));
        if (!checklistResponse.ok)
          throw new Error(
            checklistPayload?.error || "Failed to save admin checklist setting",
          );
      }
      const nextStripeSecretSet =
        form.stripeCustomerSecretKeySet ||
        Boolean(form.stripeCustomerSecretKeyInput.trim());
      const nextStripeWebhookSet =
        form.stripeCustomerWebhookSecretSet ||
        Boolean(form.stripeCustomerWebhookSecretInput.trim());
      const nextYocoSecretSet =
        form.yocoCustomerSecretKeySet ||
        Boolean(form.yocoCustomerSecretKeyInput.trim());
      ...
      const nextDarajaConsumerSecretSet =
        form.darajaConsumerSecretSet ||
        Boolean(form.darajaConsumerSecretInput.trim());
      const nextDarajaPasskeySet =
        form.darajaPasskeySet || Boolean(form.darajaPasskeyInput.trim());
      const savedPayload = {
        ...form,
        storefrontTheme: theme,
        stripeConnectionStatus: stripeCredentialReady
          ? form.stripeConnectionStatus === "not_configured"
            ? "configured"
            : form.stripeConnectionStatus || "configured"
          : "not_configured",
        stripeCustomerSecretKeyInput: "",
        stripeCustomerSecretKeySet: nextStripeSecretSet,
        stripeCustomerSecretKeyHint: nextStripeSecretSet
          ? form.stripeCustomerSecretKeyHint || "saved"
          : "",
        stripeCustomerWebhookSecretInput: "",
        stripeCustomerWebhookSecretSet: nextStripeWebhookSet,
        stripeCustomerWebhookSecretHint: nextStripeWebhookSet
          ? form.stripeCustomerWebhookSecretHint || "saved"
          : "",
        yocoConnectionStatus: yocoCredentialReady
          ? form.yocoConnectionStatus === "not_configured"
            ? "configured"
            : form.yocoConnectionStatus || "configured"
          : "not_configured",
        yocoCustomerSecretKeyInput: "",
        yocoCustomerSecretKeySet: nextYocoSecretSet,
        yocoCustomerSecretKeyHint: nextYocoSecretSet
          ? form.yocoCustomerSecretKeyHint || "saved"
          : "",
        yocoCustomerWebhookSecretInput: "",
        yocoCustomerWebhookSecretSet: nextYocoWebhookSet,
        yocoCustomerWebhookSecretHint: nextYocoWebhookSet
          ? form.yocoCustomerWebhookSecretHint || "saved"
          : "",
        ...
        mpesaConnectionStatus: mpesaCredentialReady
          ? form.mpesaConnectionStatus === "not_configured"
            ? "configured"
            : form.mpesaConnectionStatus || "configured"
          : "not_configured",
        mpesaCustomerConsumerSecretInput: "",
        mpesaCustomerConsumerSecretSet: nextMpesaConsumerSecretSet,
        mpesaCustomerConsumerSecretHint: nextMpesaConsumerSecretSet
          ? form.mpesaCustomerConsumerSecretHint || "saved"
          : "",
        mpesaCustomerPaymentsLive:
          form.enableMpesaCustomerPayments &&
          form.mpesaCustomerPaymentsLive &&
          nextMpesaConsumerSecretSet &&
          mpesaCurrencyAllowed,
        darajaConnectionStatus: darajaCredentialReady
          ? form.darajaConnectionStatus === "not_configured"
            ? "configured"
            : form.darajaConnectionStatus || "configured"
          : "not_configured",
        darajaConsumerSecretInput: "",
        darajaConsumerSecretSet: nextDarajaConsumerSecretSet,
        darajaConsumerSecretHint: nextDarajaConsumerSecretSet
          ? form.darajaConsumerSecretHint || "saved"
          : "",
        darajaPasskeyInput: "",
        darajaPasskeySet: nextDarajaPasskeySet,
        darajaPasskeyHint: nextDarajaPasskeySet
          ? form.darajaPasskeyHint || "saved"
          : "",
        darajaPaymentsLive:
          form.enableDarajaCustomerPayments &&
          form.darajaPaymentsLive &&
          nextDarajaConsumerSecretSet &&
          nextDarajaPasskeySet &&
          darajaLiveReadinessOk,
      };
  ...
  async function registerYocoWebhook() {
    ...
    try {
      const response = await fetch("/api/admin/settings/yoco-webhook", {
        method: "POST",
      });
      const payload = await response.json().catch(() => ({}));
      if (!response.ok)
        throw new Error(payload?.error || "Failed to register Yoco webhook");
      ...
      const nextForm = {
        ...
        yocoCustomerWebhookId:
          payload?.webhookId || form.yocoCustomerWebhookId || "",
        yocoCustomerWebhookUrl:
          payload?.webhookUrl ||
      ...
      const successMessage =
        payload?.message || "Yoco webhook registered and saved.";
      setTone("success");
      setMessage(successMessage);
      showToast(successMessage, "success");
    } catch (error) {
      const errorMessage =
        error instanceof Error
          ? error.message
          : "Failed to register Yoco webhook";
      setTone("error");
      setMessage(errorMessage);
      showToast(errorMessage, "error");
    } finally {
  ...
  async function runMpesaDiagnostic(
    action: "check" | "create_order" | "mark_failed",
  ) {
    const reference = mpesaDiagnosticReference.trim();
    if (!reference) {
      setMpesaDiagnosticResult({
        error:
          "Enter a checkout ID, Pesapal OrderTrackingId, or merchant reference first.",
      });
      return;
    }
    ...
    try {
      ...
      const payload = (await response
        .json()
        .catch(() => ({}))) as MpesaDiagnosticsResult;
      if (!response.ok)
        throw new Error(
          payload.error || "Could not run M-Pesa/Pesapal diagnostic check.",
        );
      ...
      setMpesaDiagnosticResult({
        error:
          error instanceof Error
            ? error.message
            : "Could not run M-Pesa/Pesapal diagnostic check.",
      });
    } finally {
        ...
        <div className="flex h-full flex-col justify-between gap-4">
          <div>
            <p className="text-sm font-semibold text-slate-900">
              Suggested logo palette
            </p>
            <p className="mt-1 text-xs leading-5 text-slate-600">
              Create or refresh a selectable palette from the uploaded logo.
            </p>
          </div>
          <button
            type="button"
          ...
          >
            {generatingLogoPalette
              ? "Generating..."
              : logoPalettePreset
                ? "Refresh logo palette"
                : "Generate from logo"}
          </button>
        </div>
      </div>
      ...
      >
        ...
        <div className="mt-3 grid grid-cols-[1fr_38px_auto] gap-2">
          ...
          <input
            type="color"
            value={
              /^#[0-9a-fA-F]{6}$/.test(customSuggestedHex)
                ? customSuggestedHex
                : "#ffffff"
            }
            onChange={(event) =>
          ...
          <button
            type="button"
            onClick={addCustomSuggestedColour}
  ...
  return (
    <div className={`mx-auto w-full max-w-7xl lg:max-w-none ${focusSection === "per-item-storefront-colours" ? "px-[25px] lg:px-[25px]" : "lg:px-[150px]"}`} data-settings-focus={focusSection || undefined}>
      ...
      >
        ...
        >
          <div className="rounded-[22px] border border-slate-300 bg-white p-4">
            <ToggleRow
              ...
              onChange={(checked) =>
                update("showAdminLaunchChecklist", checked)
              }
            />
          </div>
        </Section>
                ...
                {form.faviconUrl ? (
                  <div className="flex items-center gap-3 rounded-2xl border border-slate-100 bg-slate-50 p-4 text-xs text-slate-600">
                    <img
                      ...
                      className="h-12 w-12 rounded-lg border border-slate-100 bg-white object-contain"
                    />
                    <span>
                      Current favicon preview. Browser tabs may need a hard
                      refresh before the new icon appears.
                    </span>
                  </div>
                ) : (
                  <div className="flex min-h-28 items-center justify-center rounded-2xl border border-dashed border-slate-200 bg-slate-50 px-4 py-5 text-center text-xs font-semibold text-slate-500">
        ...
        <Section
          id="branding-and-wording"
          title="Branding and wording"
          dirty={brandingDirty}
          saving={saving}
        >
          <div className="grid gap-4 md:grid-cols-2">
            <Field label="Business display name">
              <input
                ...
                onChange={(e) => update("businessDisplayName", e.target.value)}
                className="input"
            ...
            <Field label="Admin heading label">
              <input
                value={form.adminHeadingLabel}
                onChange={(e) => update("adminHeadingLabel", e.target.value)}
                className="input"
                placeholder="Used in the admin shell"
              />
            </Field>
            <div className="md:col-span-2">
              <Field label="Storefront heading">
                <input
                  value={form.storefrontHeading}
                  onChange={(e) => update("storefrontHeading", e.target.value)}
                  className="input"
            ...
            <div className="md:col-span-2">
              <Field label="Storefront subheading">
                <textarea
                  value={form.storefrontSubheading}
                  onChange={(e) =>
                    update("storefrontSubheading", e.target.value)
                  }
                  rows={3}
                  className="input"
                  placeholder="A short welcome line for this business"
                />
              </Field>
            </div>
          </div>
        ...
        >
          <div className="mb-4 rounded-[22px] border border-orange-100 bg-orange-50/70 p-4">
            <p className="text-sm font-semibold text-slate-900">
              Active preset:{" "}
              {activePreset
                ? `${activePreset.name}${theme.customised ? " — customised" : ""}`
                : "Custom"}
            </p>
            <p className="mt-1 text-xs leading-5 text-slate-600">
            ...
            {availableThemePresets.map((preset) => {
              ...
              return (
                <Fragment key={preset.name}>
                  ...
                  {preset.name === LOGO_PALETTE_PRESET_NAME
                    ? renderLogoPaletteGeneratorCard()
                    : null}
                </Fragment>
              );
            })}
        ...
        <Section
          id="storefront-additions"
          title="Storefront welcome & About us"
          dirty={themeDirty}
          saving={saving}
        >
                ...
                return (
                  ...
                  >
                    ...
                    >
                      ...
                      <span
                        className={`mt-0.5 inline-flex h-8 w-8 shrink-0 items-center justify-center rounded-full border text-sm font-bold transition ${isOpen ? "border-orange-200 bg-orange-50 text-orange-800" : "border-slate-200 bg-slate-50 text-slate-500"}`}
                        aria-hidden="true"
                      >
                        {isOpen ? "−" : "+"}
                      </span>
        ...
        >
          <div className="grid gap-4 md:grid-cols-2">
            ...
            <div className="md:col-span-2">
              <Field label="Footer notice">
                <input
                  value={form.footerNotice}
                  onChange={(e) => update("footerNotice", e.target.value)}
                  className="input"
                  placeholder="Prices and availability may change without notice."
                />
              </Field>
            </div>
              ...
              <label className="flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between">
                <span>
                  <span className="block text-sm font-black text-slate-900">
                    Show Orduva referral advert
                  </span>
                  <span className="mt-1 block text-xs leading-5 text-slate-600">
                    Adds a tasteful “Do you need a store like this?” advert at
                    the bottom of the storefront. Later, signups from this link
                    can be tied to tenant referral rewards.
                  </span>
                </span>
                <input
                  type="checkbox"
                  ...
                  onChange={(e) =>
                    update("showOrduvaReferralAd", e.target.checked)
                  }
          ...
          <div className="mt-5 rounded-[22px] border border-slate-100 bg-slate-50 p-4">
            ...
            <p className="mt-1 text-xs leading-5 text-slate-500">
              These appear in the storefront footer as centred icon-only links,
              with up to 8 icons shown in rows of 4. Leave blank to hide an
              icon.
            </p>
            <div className="mt-4 grid gap-4 md:grid-cols-2">
              <Field label="Facebook URL">
                <input
                  ...
                  onChange={(e) => update("socialFacebookUrl", e.target.value)}
                  className="input"
              ...
              <Field label="Instagram URL">
                <input
                  ...
                  onChange={(e) => update("socialInstagramUrl", e.target.value)}
                  className="input"
              ...
              <Field label="TikTok URL">
                <input
                  ...
                  onChange={(e) => update("socialTikTokUrl", e.target.value)}
                  className="input"
              ...
              <Field label="X / Twitter URL">
                <input
                  ...
                  onChange={(e) => update("socialXUrl", e.target.value)}
                  className="input"
        ...
        <Section
          ...
          dirty={invoicePaymentsDirty}
          saving={saving}
        >
          <div className="rounded-[22px] border border-blue-200 bg-blue-50/80 p-4 text-sm leading-6 text-blue-950">
            <p className="font-semibold text-slate-950">
              Let customers pay invoices, deposits and statement balances
            </p>
            <p className="mt-1 text-xs leading-5 text-blue-900">
              When enabled, customer-entered amount payment cards are shown in
              their own first storefront section. These cards use a separate
              layout with no product image, variants, favourites or normal
          ...
          <div className="mt-4 grid gap-4 md:grid-cols-2">
            <div className="md:col-span-2 rounded-[22px] border border-slate-200 bg-white p-4">
              <label className="flex items-start justify-between gap-4">
                <span>
                  ...
                  <span className="mt-1 block text-xs leading-5 text-slate-600">
                    Switch this on to show a dedicated Payments section on the
                    storefront. These payment cards are managed here, not
                    through Add Products.
                  </span>
                </span>
                <input
                  type="checkbox"
                  checked={form.invoicePaymentsEnabled}
                  onChange={(e) =>
                    update("invoicePaymentsEnabled", e.target.checked)
                  }
            ...
            <Field label="Section title">
              <input
                value={form.invoicePaymentsSectionTitle}
                onChange={(e) =>
                  update("invoicePaymentsSectionTitle", e.target.value)
                }
                className="input"
            ...
            <Field label="Intro text">
              <input
                value={form.invoicePaymentsIntroText}
                onChange={(e) =>
                  update("invoicePaymentsIntroText", e.target.value)
                }
                className="input"
                placeholder="Pay an invoice, deposit or statement balance securely online."
              />
            </Field>
            <div className="md:col-span-2 rounded-[22px] border border-slate-200 bg-slate-50 p-4">
              <p className="text-sm font-semibold text-slate-900">
                Payment cards to show
              </p>
              <p className="mt-1 text-xs leading-5 text-slate-600">
                These are dedicated payment cards. They do not use product
                images, variants, stock, rewards, discounts or the normal cart.
              </p>
              <div className="mt-4 grid gap-3 md:grid-cols-3">
                <label className="flex items-center gap-3 rounded-2xl border border-slate-200 bg-white px-4 py-3 text-sm font-semibold text-slate-800">
                  <input
                    type="checkbox"
                    checked={form.invoicePaymentsInvoiceEnabled}
                    onChange={(e) =>
                      update("invoicePaymentsInvoiceEnabled", e.target.checked)
                    }
                ...
                <label className="flex items-center gap-3 rounded-2xl border border-slate-200 bg-white px-4 py-3 text-sm font-semibold text-slate-800">
                  <input
                    type="checkbox"
                    checked={form.invoicePaymentsDepositEnabled}
                    onChange={(e) =>
                      update("invoicePaymentsDepositEnabled", e.target.checked)
                    }
                    className="h-5 w-5 rounded border-slate-300 text-blue-700 focus:ring-blue-200"
                  />
                  Pay a Deposit
                ...
                <label className="flex items-center gap-3 rounded-2xl border border-slate-200 bg-white px-4 py-3 text-sm font-semibold text-slate-800">
                  <input
                    type="checkbox"
                    checked={form.invoicePaymentsBalanceEnabled}
                    onChange={(e) =>
                      update("invoicePaymentsBalanceEnabled", e.target.checked)
                    }
                    className="h-5 w-5 rounded border-slate-300 text-blue-700 focus:ring-blue-200"
                  />
        ...
        >
          <div className="rounded-[22px] border border-emerald-200 bg-[#EAF3FA]/80 p-4 text-sm leading-6 text-emerald-950">
            ...
            <p className="mt-1 text-xs leading-5 text-emerald-900">
              These details appear near the order number on the customer receipt
              and generated PDF. Leave optional fields blank, or unticked, to
              hide them.
            </p>
          </div>
          ...
          <div className="mt-4 grid gap-4 md:grid-cols-2">
            <Field label="Document name">
              <input
                value={form.receiptDocumentName}
                onChange={(e) => update("receiptDocumentName", e.target.value)}
                className="input"
                placeholder="Receipt"
              />
            </Field>
            <Field label="Tax label">
              <select
                value={form.receiptTaxLabel}
                onChange={(e) =>
                  update(
                    "receiptTaxLabel",
                    e.target.value as FormState["receiptTaxLabel"],
                  )
                }
                className="input"
              >
            ...
            <Field label="Tax number">
              <input
                value={form.receiptTaxNumber}
                onChange={(e) => update("receiptTaxNumber", e.target.value)}
                className="input"
            ...
            <Field label="VAT / GST rate %">
              <input
                value={form.receiptTaxRatePercent}
                onChange={(e) =>
                  update("receiptTaxRatePercent", e.target.value)
                }
                className="input"
                inputMode="decimal"
                placeholder="0"
            ...
            <Field label="Receipt image">
              <select
                value={form.receiptBrandImageMode}
                onChange={(e) =>
                  update(
                    "receiptBrandImageMode",
                    e.target.value as FormState["receiptBrandImageMode"],
                  )
                }
                className="input"
              >
            ...
            <div className="md:col-span-2 grid gap-4 rounded-[22px] border border-slate-200 bg-slate-50 p-4 md:grid-cols-[auto_1fr_1fr]">
              <label className="flex items-center gap-3 text-sm font-bold text-slate-800">
                <input
                  type="checkbox"
                  checked={form.receiptExtraField1Enabled}
                  onChange={(e) =>
                    update("receiptExtraField1Enabled", e.target.checked)
                  }
                  className="h-5 w-5 rounded border-slate-300 text-emerald-600 focus:ring-emerald-200"
                />
                Show field 1
              </label>
              <Field label="Field 1 label">
                <input
                  value={form.receiptExtraField1Label}
                  onChange={(e) =>
                    update("receiptExtraField1Label", e.target.value)
                  }
                  className="input"
                  placeholder="Company Reg"
                />
              </Field>
              <Field label="Field 1 value">
                <input
                  value={form.receiptExtraField1Value}
                  onChange={(e) =>
                    update("receiptExtraField1Value", e.target.value)
                  }
                  className="input"
                  placeholder="Optional receipt detail"
                />
              </Field>
            </div>
            ...
            <div className="md:col-span-2 grid gap-4 rounded-[22px] border border-slate-200 bg-slate-50 p-4 md:grid-cols-[auto_1fr_1fr]">
              <label className="flex items-center gap-3 text-sm font-bold text-slate-800">
                <input
                  type="checkbox"
                  checked={form.receiptExtraField2Enabled}
                  onChange={(e) =>
                    update("receiptExtraField2Enabled", e.target.checked)
                  }
                  className="h-5 w-5 rounded border-slate-300 text-emerald-600 focus:ring-emerald-200"
                />
                Show field 2
              </label>
              <Field label="Field 2 label">
                <input
                  value={form.receiptExtraField2Label}
                  onChange={(e) =>
                    update("receiptExtraField2Label", e.target.value)
                  }
                  className="input"
                  placeholder="Licence No."
                />
              </Field>
              <Field label="Field 2 value">
                <input
                  value={form.receiptExtraField2Value}
                  onChange={(e) =>
                    update("receiptExtraField2Value", e.target.value)
                  }
                  className="input"
                  placeholder="Optional receipt detail"
                />
            ...
            <div className="md:col-span-2">
              <Field label="Receipt footer message">
                <textarea
                  ...
                  onChange={(e) =>
                    update("receiptFooterMessage", e.target.value)
                  }
                  className="input min-h-28 resize-y"
                  placeholder="Thank you for your order. Add returns, tax, contact or business wording here."
                />
              </Field>
            </div>
          </div>
        ...
        >
          <div className="rounded-[22px] border border-blue-200 bg-blue-50/80 p-4 text-sm leading-6 text-blue-950">
            <p className="font-black text-slate-950">
              Search, social and tracking basics
            </p>
            <p className="mt-1 text-xs leading-5 text-blue-900">
              These settings improve the public storefront page title,
              description, browser favicon, structured data, and optional Google
              tracking. They do not change checkout, prices or payment
              behaviour.
            </p>
          </div>
          ...
          <div className="mt-4 grid gap-4 md:grid-cols-2">
            <div className="md:col-span-2">
              <Field label={`Page name (${form.seoPageName.length}/55)`}>
                <input
                  maxLength={55}
                  value={form.seoPageName}
                  onChange={(e) =>
                    update("seoPageName", e.target.value.slice(0, 55))
                  }
                  className="input"
                  placeholder={`${previewName} | Online Ordering`}
                />
              </Field>
              <p className="mt-1 text-xs text-slate-500">
                Shown as the storefront browser title and search result title.
                Maximum 55 characters including spaces and punctuation.
              </p>
            </div>

            <div className="md:col-span-2">
              <Field
                label={`Page meta description (${form.seoMetaDescription.length}/160)`}
              >
                <textarea
                  maxLength={160}
                  value={form.seoMetaDescription}
                  onChange={(e) =>
                    update("seoMetaDescription", e.target.value.slice(0, 160))
                  }
                  className="input min-h-24 resize-y"
                  placeholder="Describe what customers can order, where you serve, and why they should choose you."
                />
              </Field>
              <p className="mt-1 text-xs text-slate-500">
                Recommended maximum 160 characters including spaces and
                punctuation.
              </p>
            </div>
            ...
            <Field label="SEO keywords / phrases">
              <input
                ...
                className="input"
                placeholder="restaurant, takeaway, groceries, Nairobi"
              />
            </Field>
            ...
            <div>
              <Field label="Canonical URL">
                <input
                  ...
                  className="input"
                  placeholder="https://store.orduva.com/"
                />
              </Field>
              <p className="mt-1 text-xs leading-5 text-slate-500">
                Leave this blank unless you use your own custom domain. If you
                do, enter the main website address customers should find on
                Google.
              </p>
            </div>
            ...
            <div className="md:col-span-2 rounded-[22px] border border-slate-200 bg-slate-50 p-4">
              <label className="flex items-start justify-between gap-4">
                <span>
                  ...
                  <span className="mt-1 block text-xs leading-5 text-slate-600">
                    Adds JSON-LD to the storefront so search engines and AI
                    systems can identify the business name, contact details,
                    logo, icon, address and ordering website.
                  </span>
                </span>
                <input
                  type="checkbox"
                  checked={form.seoStructuredDataEnabled}
                  onChange={(e) =>
                    update("seoStructuredDataEnabled", e.target.checked)
                  }
            ...
            <div className="md:col-span-2 rounded-[22px] border border-slate-200 bg-white p-4">
              ...
              <p className="mt-1 text-xs leading-5 text-slate-600">
                To enable Google Analytics or Google Ads on your site, enter
                your Google tracking ID below. This will automatically send page
                views and basic order conversion events to Google where
                available.
              </p>
              <div className="mt-4">
                <Field label="Google tracking ID">
                  <input
                    value={form.googleTrackingId}
                    onChange={(e) => update("googleTrackingId", e.target.value)}
                    className="input"
                    placeholder="UA-XXXX-XX, G-XXXXXX or AW-XXXXXX"
                  />
                </Field>
              </div>
            </div>
            ...
            <div className="md:col-span-2 rounded-[22px] border border-slate-200 bg-white p-4">
              ...
              <p className="mt-1 text-xs leading-5 text-slate-600">
                This allows you to install and manage different marketing tags
                without any use of code. Enter your container ID to enable
                Google Tag Manager on your site.
              </p>
              <div className="mt-4">
                <Field label="Google Tag Manager container ID">
                  <input
                    ...
                    onChange={(e) =>
                      update("googleTagManagerId", e.target.value)
                    }
                    className="input"
        ...
        >
          <div className="rounded-[22px] border border-emerald-200 bg-[#EAF3FA]/80 p-4 text-sm leading-6 text-emerald-950">
            ...
            <p className="mt-1 text-xs leading-5 text-emerald-900">
              Every customer account is automatically enrolled when this is
              switched on. Their tier is calculated from their previous
              qualifying spend with this store. The tier discount is applied
              before future discount codes are added in a later build.
            </p>
          </div>
          ...
          <div className="mt-4 grid gap-4 md:grid-cols-2">
            <div className="md:col-span-2 rounded-[22px] border border-slate-200 bg-white p-4">
              <label className="flex items-start justify-between gap-4">
                <span>
                  <span className="block text-sm font-black text-slate-900">
                    Enable rewards programme
                  </span>
                  <span className="mt-1 block text-xs leading-5 text-slate-600">
                    Shows a compact rewards icon on the customer welcome panel
                    and applies the customer tier discount at checkout when
                    signed in.
                  </span>
                </span>
                <input
            ...
            <Field label="Programme name">
              <input
                value={form.rewardsProgramName}
                onChange={(e) => update("rewardsProgramName", e.target.value)}
                className="input"
                placeholder="Rewards Club"
              />
            </Field>
            <Field label="Silver discount %">
              <input
                type="number"
                min={0}
                max={95}
                step="0.1"
                value={form.rewardsSilverDiscountPercent}
                onChange={(e) =>
                  update("rewardsSilverDiscountPercent", e.target.value)
                }
                className="input"
              />
            </Field>
            <Field label="Gold spend requirement">
              <input
                type="number"
                min={0}
                step="1"
                value={form.rewardsGoldMinSpend}
                onChange={(e) => update("rewardsGoldMinSpend", e.target.value)}
                className="input"
              />
            </Field>
            <Field label="Gold discount %">
              <input
                type="number"
                min={0}
                max={95}
                step="0.1"
                value={form.rewardsGoldDiscountPercent}
                onChange={(e) =>
                  update("rewardsGoldDiscountPercent", e.target.value)
                }
                className="input"
              />
            </Field>
            <Field label="Platinum spend requirement">
              <input
                type="number"
                min={0}
                step="1"
                value={form.rewardsPlatinumMinSpend}
                onChange={(e) =>
                  update("rewardsPlatinumMinSpend", e.target.value)
                }
                className="input"
              />
            </Field>
            <Field label="Platinum discount %">
              <input
                type="number"
                min={0}
                max={95}
                step="0.1"
                value={form.rewardsPlatinumDiscountPercent}
                onChange={(e) =>
                  update("rewardsPlatinumDiscountPercent", e.target.value)
                }
                className="input"
              />
            </Field>
          ...
          <div className="mt-5 grid gap-3 md:grid-cols-3">
            <RewardTierPreview
              name="Silver"
              spend="Automatic"
              discount={form.rewardsSilverDiscountPercent || "0"}
              tone="silver"
            />
            <RewardTierPreview
              name="Gold"
              spend={`${form.rewardsGoldMinSpend || "0"}+`}
              discount={form.rewardsGoldDiscountPercent || "0"}
              tone="gold"
            />
            <RewardTierPreview
              name="Platinum"
              spend={`${form.rewardsPlatinumMinSpend || "0"}+`}
              discount={form.rewardsPlatinumDiscountPercent || "0"}
              tone="platinum"
            />
          </div>

          <p className="mt-4 text-xs leading-5 text-slate-500">
            Discounts are managed in the next section. Each discount can decide
            whether it stacks with this rewards discount or replaces it.
          </p>
        </Section>

        <Section
          id="discounts-and-codes"
          title="Discounts & discount codes"
        ...
        >
          <div className="rounded-[22px] border border-emerald-200 bg-[#EAF3FA]/80 p-4 text-sm leading-6 text-emerald-950">
            ...
            <p className="mt-1 text-xs leading-5 text-emerald-900">
              Create a code to share with a customer, attach an offer to one
              product, require a combination of up to three products, or run a
              site-wide campaign with a date range. Each discount can decide
              whether it stacks with rewards.
            </p>
          </div>
          ...
          <div className="mt-4 grid gap-4 md:grid-cols-2">
            <label className="flex items-start justify-between gap-4 rounded-[22px] border border-slate-200 bg-white p-4">
              <span>
                ...
                <span className="mt-1 block text-xs leading-5 text-slate-600">
                  Allows active discounts to appear at checkout and be applied
                  to the order total.
                </span>
              </span>
              <input
                ...
                onChange={(e) => update("discountsEnabled", e.target.checked)}
                className="mt-1 h-5 w-5 rounded border-slate-300 text-emerald-600 focus:ring-emerald-200"
              />
            </label>
            <label className="flex items-start justify-between gap-4 rounded-[22px] border border-slate-200 bg-white p-4">
              <span>
                <span className="block text-sm font-black text-slate-900">
                  Show discount popup on loading
                </span>
                <span className="mt-1 block text-xs leading-5 text-slate-600">
                  Shows a tasteful offers popup when the storefront loads, if
                  there are active visible offers.
                </span>
              </span>
              <input
                type="checkbox"
                checked={form.discountPopupEnabled}
                onChange={(e) =>
                  update("discountPopupEnabled", e.target.checked)
                }
                className="mt-1 h-5 w-5 rounded border-slate-300 text-emerald-600 focus:ring-emerald-200"
              />
            </label>
            <Field label="Popup title">
              <input
                value={form.discountPopupTitle}
                onChange={(e) => update("discountPopupTitle", e.target.value)}
                className="input"
                placeholder="Today’s offers"
              />
            </Field>
            <Field label="Popup message">
              <input
                value={form.discountPopupMessage}
                onChange={(e) => update("discountPopupMessage", e.target.value)}
                className="input"
                placeholder="Tap an offer at checkout to apply it."
              />
            </Field>
          </div>
          ...
          <div className="mt-5 flex flex-col gap-3 rounded-[22px] border border-slate-200 bg-white p-4 sm:flex-row sm:items-center sm:justify-between">
            <div>
              ...
              <p className="mt-1 text-xs leading-5 text-slate-600">
                Use product rules for one product, combo rules for up to three
                products, or site-wide rules for the full basket. Leave code
                blank for an automatic visible offer.
              </p>
            </div>
            <button
              type="button"
              onClick={addDiscountRule}
              className="inline-flex min-h-10 items-center justify-center rounded-2xl bg-slate-950 px-4 py-2 text-xs font-black text-white shadow-sm"
            >
              Add discount
            </button>
          </div>
          ...
          <div className="mt-4 space-y-4">
            {(form.discountRules || []).length ? (
              form.discountRules.map((rule, index) => (
                <div
                  key={rule.id || index}
                  className="rounded-[24px] border border-emerald-300 bg-emerald-100/70 p-4"
                >
                  <div className="flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between">
                    <div>
                      <p className="text-sm font-semibold text-slate-950">
                      ...
                      <p className="mt-1 text-xs leading-5 text-slate-600">
                        ...
                        {rule.type === "percentage"
                          ? `${rule.value || 0}%`
                          : `${form.currencySymbol || ""}${rule.value || 0}`}
                      </p>
                    </div>
                    <button
                      type="button"
                      onClick={() => removeDiscountRule(index)}
                      className="inline-flex min-h-9 items-center justify-center rounded-full border border-rose-200 bg-white px-3 py-2 text-[11px] font-black text-rose-700"
                    >
                      Remove
                    </button>
                  </div>
                  <div className="mt-4 grid gap-3 md:grid-cols-2">
                    ...
                    <Field label="Code to copy/paste">
                      <input
                        value={rule.code}
                        onChange={(e) =>
                          updateDiscountRule(index, {
                    ...
                    <Field label="Discount type">
                      <select
                        ...
                        onChange={(e) =>
                          updateDiscountRule(index, {
                            type: e.target.value as DiscountRuleForm["type"],
                          })
                        }
                        className="input"
                      >
                    ...
                    >
                      <input
                        type="number"
                        min={0}
                        max={rule.type === "percentage" ? 95 : undefined}
                        step="0.1"
                        value={rule.value}
                        onChange={(e) =>
                          updateDiscountRule(index, { value: e.target.value })
                        }
                        className="input"
                      />
                    </Field>
                    <Field label="Applies to">
                      <select
                        ...
                        onChange={(e) =>
                          updateDiscountRule(index, {
                            scope: e.target.value as DiscountRuleForm["scope"],
                            productIds:
                        ...
                        className="input"
                      >
                        <option value="sitewide">Site-wide basket</option>
                        <option value="product">Specific product</option>
                        <option value="combo">Combination of products</option>
                      </select>
                    </Field>
                    ...
                    <Field label="End date">
                      <input
                        ...
                        value={rule.endsAt}
                        onChange={(e) =>
                          updateDiscountRule(index, { endsAt: e.target.value })
                        }
                        className="input"
                      />
                    </Field>
                  ...
                  {rule.scope !== "sitewide" ? (
                    <div className="mt-4 rounded-[22px] border border-emerald-200 bg-white/75 p-3">
                      <p className="text-xs font-black uppercase tracking-[0.14em] text-emerald-900">
                        Choose products{" "}
                        {rule.scope === "combo"
                          ? "for this bundle, up to 3"
                          : "for this offer"}
                      </p>
                      <div className="mt-3 grid gap-2 sm:grid-cols-2">
                        {discountProductOptions.length ? (
                          discountProductOptions.map((product) => (
                            <label
                              key={product.id}
                              className="flex items-start gap-2 rounded-2xl border border-slate-200 bg-white px-3 py-2 text-xs text-slate-700"
                            >
                              <input
                                type="checkbox"
                                checked={(rule.productIds || []).includes(
                                  product.id,
                                )}
                                onChange={(e) =>
                                ...
                                disabled={
                                  !(rule.productIds || []).includes(
                                    product.id,
                                  ) && (rule.productIds || []).length >= 3
                                }
                          ...
                          <p className="text-xs text-slate-500">
                            Products will appear here after the menu has
                            products.
                          </p>
                        )}
                      </div>
                  ...
                  <div className="mt-4 grid gap-3 md:grid-cols-2">
                    <label className="flex items-start gap-3 rounded-2xl border border-slate-200 bg-white p-3 text-sm text-slate-700">
                      ...
                      <span>
                        ...
                        <span className="block text-xs">
                          Inactive discounts are saved but ignored.
                        </span>
                      </span>
                    </label>
                    <label className="flex items-start gap-3 rounded-2xl border border-slate-200 bg-white p-3 text-sm text-slate-700">
                      <input
                        type="checkbox"
                        checked={rule.showOnCheckout}
                        onChange={(e) =>
                          updateDiscountRule(index, {
                            showOnCheckout: e.target.checked,
                          })
                        }
                        className="mt-1 h-5 w-5 rounded border-slate-300 text-emerald-600 focus:ring-emerald-200"
                      />
                      <span>
                        ...
                        <span className="block text-xs">
                          Customers can tap Apply instead of typing the code.
                        </span>
                      </span>
                    </label>
                    <label className="flex items-start gap-3 rounded-2xl border border-slate-200 bg-white p-3 text-sm text-slate-700">
                      <input
                        type="checkbox"
                        checked={rule.allowWithRewards}
                        onChange={(e) =>
                          updateDiscountRule(index, {
                            allowWithRewards: e.target.checked,
                          })
                        }
                        className="mt-1 h-5 w-5 rounded border-slate-300 text-emerald-600 focus:ring-emerald-200"
                      />
                      <span>
                        ...
                        <span className="block text-xs">
                          Turn off if this discount replaces the customer tier
                          reward.
                        </span>
                      </span>
                    </label>
                    <label className="flex items-start gap-3 rounded-2xl border border-slate-200 bg-white p-3 text-sm text-slate-700">
                      <input
                        type="checkbox"
                        checked={rule.onlyThisDiscount}
                        onChange={(e) =>
                          updateDiscountRule(index, {
                            onlyThisDiscount: e.target.checked,
                            allowWithRewards: e.target.checked
                              ? false
                              : rule.allowWithRewards,
                          })
                        }
                        className="mt-1 h-5 w-5 rounded border-slate-300 text-emerald-600 focus:ring-emerald-200"
                      />
                      <span>
                        <strong className="text-slate-900">
                          Only this discount applies
                        </strong>
                        <span className="block text-xs">
                          Blocks reward stacking and future promo stacking.
                        </span>
                      </span>
                    </label>
                    <label className="flex items-start gap-3 rounded-2xl border border-slate-200 bg-white p-3 text-sm text-slate-700 md:col-span-2">
                      <input
                        type="checkbox"
                        checked={rule.popupEnabled}
                        onChange={(e) =>
                          updateDiscountRule(index, {
                      ...
                      <span>
                        <strong className="text-slate-900">
                          Feature in loading popup
                        </strong>
                        <span className="block text-xs">
                          Best for site-wide campaigns or limited-time bundles.
                        </span>
                      </span>
                    </label>
                  </div>
                  {rule.popupEnabled ? (
                    <div className="mt-4 grid gap-3 md:grid-cols-2">
                      <Field label="Offer popup title">
                        <input
                          value={rule.popupTitle}
                          onChange={(e) =>
                            updateDiscountRule(index, {
                      ...
                      <Field label="Offer popup message">
                        <input
                          ...
                          className="input"
                          placeholder="Add your favourites and apply this at checkout."
                        />
                      </Field>
                    </div>
                  ) : null}
              ...
              <div className="rounded-[22px] border border-dashed border-slate-300 bg-slate-50 p-4 text-sm leading-6 text-slate-600">
                No discounts yet. Add your first product, combo or site-wide
        ...
        >
            ...
            <div className="mx-auto w-full rounded-[22px] border border-emerald-300 bg-emerald-100/70 p-4">
              <label className="flex items-start justify-between gap-4">
                <span>
                  ...
                  <span className="mt-1 block text-xs leading-5 text-slate-600">
                    Allow customers collecting their order to pay the store
                ...
                <input
                  ...
                  onChange={(e) =>
                    update("enableCashOnCollection", e.target.checked)
                  }
                  className="mt-1 h-5 w-5 rounded border-slate-300 text-emerald-600 focus:ring-emerald-200"
                />
              </label>
            ...
            <div className="mx-auto w-full rounded-[22px] border border-emerald-300 bg-emerald-100/70 p-4">
              <label className="flex items-start justify-between gap-4">
                <span>
                  ...
                  <span className="mt-1 block text-xs leading-5 text-slate-600">
                    Allow delivery customers to pay the store directly when the
                    order arrives.
                  </span>
                </span>
                <input
                  ...
                  onChange={(e) =>
                    update("enableCashOnDelivery", e.target.checked)
                  }
                  className="mt-1 h-5 w-5 rounded border-slate-300 text-emerald-600 focus:ring-emerald-200"
                />
              </label>
          ...
          <div className="mt-6">
            ...
            <p className="mt-2 text-xs leading-5 text-slate-600">
              These settings belong to the store owner. They do not use the
              Orduva owner Stripe account that collects SaaS subscriptions.
            </p>
            <div className="mt-3 rounded-2xl border border-amber-200 bg-amber-50 px-4 py-3 text-xs leading-5 text-amber-950">
              <strong>Important:</strong> each store must use its own payment
              provider account. For Stripe, use the tenant's own Stripe keys,
              not the Orduva subscription billing keys.
            </div>
          </div>
              ...
              <div className="space-y-4 text-sm">
                <div className="flex flex-col gap-3 md:flex-row md:items-start md:justify-between">
                  <div>
                    ...
                    <p className="mt-1 text-xs leading-5 text-slate-600">
                      Save this tenant's own Stripe keys and webhook secret.
                      Once enabled, Stripe appears as a customer payment option
                      on the storefront checkout.
                    </p>
                  </div>
                ...
                <div className="mt-4 grid gap-3 md:grid-cols-2">
                  <Field label="Stripe setup mode">
                    <select
                      value={form.stripeCustomerPaymentMode}
                      onChange={(e) =>
                        update(
                          "stripeCustomerPaymentMode",
                          e.target
                            .value as FormState["stripeCustomerPaymentMode"],
                        )
                      }
                      className="input"
                    >
                  ...
                  <Field label="Account label / business name">
                    <input
                      value={form.stripeCustomerAccountLabel}
                      onChange={(e) =>
                        update("stripeCustomerAccountLabel", e.target.value)
                      }
                      placeholder="Example: ZimZa Express Stripe"
                      className="input"
                    />
                  </Field>
                  <Field label="Store Stripe publishable key">
                    <input
                      ...
                      onChange={(e) =>
                        update("stripeCustomerPublishableKey", e.target.value)
                      }
                  ...
                  >
                    <input
                      value={form.stripeCustomerSecretKeyInput}
                      onChange={(e) =>
                        update("stripeCustomerSecretKeyInput", e.target.value)
                      }
                      placeholder={
                        form.stripeCustomerSecretKeySet
                  ...
                  >
                    <input
                      value={form.stripeCustomerWebhookSecretInput}
                      onChange={(e) =>
                        update(
                          "stripeCustomerWebhookSecretInput",
                          e.target.value,
                    ...
                    <p className="mt-2 text-xs leading-5 text-amber-700">
                      ...
                      https://www.orduva.com/api/storefront/stripe/webhook
                    </p>
                  </Field>
                  <Field label="Setup notes">
                    <input
                      value={form.stripeCustomerSetupNotes}
                      onChange={(e) =>
                ...
                <div className="mt-4 grid gap-3 md:grid-cols-2">
                  <label className="flex items-start gap-3 rounded-2xl border border-slate-200 bg-slate-50 p-3 text-sm text-slate-700">
                    <input
                      ...
                      onChange={(e) =>
                        update("stripeCustomerTestMode", e.target.checked)
                      }
                    ...
                    <span>
                      ...
                      <span className="mt-1 block text-xs leading-5 text-slate-600">
                        Use test keys until the tenant is ready for real
                        customer payments.
                      </span>
                    </span>
                  </label>
                  ...
                  >
                    <input
                      ...
                      onChange={(e) =>
                        update("enableStripeCustomerPayments", e.target.checked)
                      }
                    ...
                    />
                    <span>
                      <strong>Enable Stripe for this tenant</strong>
                      <span className="mt-1 block text-xs leading-5">
                        Requires this tenant's publishable key, secret key and
                        webhook secret. When enabled, Stripe appears on the
                        customer checkout for this store.
                      </span>
                    </span>
                  </label>
                </div>
                ...
                {!stripeCredentialReady ? (
                  <p className="mt-3 rounded-2xl border border-amber-200 bg-amber-50 px-3 py-2 text-xs font-bold text-amber-900">
                    Add this tenant's Stripe publishable key, secret key and
              ...
              <div className="space-y-4">
                <div className="flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between">
                  <div>
                    ...
                    <p className="mt-1 text-xs leading-5 text-slate-600">
                      Hosted Yoco checkout for South African ZAR stores.
                      Customer payments can be switched live once the tenant's
                      Yoco secret key has been saved and tested.
                    </p>
                  </div>
                  <span
                ...
                {!yocoCurrencyAllowed ? (
                  <p className="mt-3 rounded-2xl border border-amber-200 bg-amber-50 px-3 py-2 text-xs font-bold text-amber-900">
                    Yoco is currently intended for South African Rand stores.
                    Change the store currency to ZAR before enabling Yoco.
                  </p>
                ) : null}
                ...
                <div className="mt-4 grid gap-4 md:grid-cols-2">
                  <Field label="Yoco mode">
                    <select
                      value={form.yocoCustomerMode}
                      onChange={(e) =>
                        setYocoMode(
                          e.target.value as FormState["yocoCustomerMode"],
                        )
                      }
                      className="input"
                    >
                    ...
                    <p className="mt-2 text-xs leading-5 text-slate-500">
                      Changing mode resets the webhook and checkout switch so
                      Test and Live credentials cannot be mixed accidentally.
                    </p>
                  </Field>
                  <Field label="Yoco account label">
                    <input
                      value={form.yocoCustomerAccountLabel}
                      onChange={(e) =>
                        update("yocoCustomerAccountLabel", e.target.value)
                      }
                      placeholder="Example: Kahuna Yoco account"
                      className="input"
                    />
                  </Field>
                  <Field
                    label={
                      form.yocoCustomerSecretKeySet
                        ? `Yoco secret key saved (${form.yocoCustomerSecretKeyHint || "saved"})`
                        : "Yoco secret key"
                    }
                  >
                    <input
                      value={form.yocoCustomerSecretKeyInput}
                      onChange={(e) =>
                        update("yocoCustomerSecretKeyInput", e.target.value)
                      }
                      placeholder={
                        form.yocoCustomerSecretKeySet
                          ? "Leave blank to keep saved Yoco secret key"
                          : "Yoco secret key from the tenant's Yoco portal"
                      }
                      className="input"
                      autoComplete="off"
                    />
                  ...
                  <Field
                    label={
                      form.yocoCustomerWebhookSecretSet
                        ? `Yoco webhook secret saved (${form.yocoCustomerWebhookSecretHint || "saved"})`
                        : "Yoco webhook secret / signing key"
                    }
                  >
                    <input
                      value={form.yocoCustomerWebhookSecretInput}
                      onChange={(e) =>
                        update("yocoCustomerWebhookSecretInput", e.target.value)
                      }
                      placeholder={
                        form.yocoCustomerWebhookSecretSet
                          ? "Leave blank to keep saved webhook secret"
                          : "Use the register button below, or paste the Yoco webhook secret if already created"
                      }
                      className="input"
                      autoComplete="off"
                    />
                    ...
                    {form.yocoCustomerWebhookId ? (
                      <p className="mt-1 text-xs leading-5 text-slate-500">
                        Yoco webhook ID: {form.yocoCustomerWebhookId}
                      </p>
                    ) : null}
                  </Field>
                  <div className="md:col-span-2">
                    <Field label="Yoco setup notes">
                      <input
                        value={form.yocoCustomerSetupNotes}
                        onChange={(e) =>
                          update("yocoCustomerSetupNotes", e.target.value)
                        }
                        placeholder="Example: Test Yoco key saved, waiting for checkout build"
                        className="input"
                      />
                    </Field>
                  </div>
                ...
                <div className="mt-4 rounded-2xl border border-emerald-100 bg-white p-4">
                  <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
                    <div>
                      ...
                      <p className="mt-1 text-xs leading-5 text-slate-600">
                        Register the Orduva webhook with Yoco after the Test
                        Secret Key is saved. This lets Yoco confirm paid orders
                        even if the customer closes the browser.
                      </p>
                    </div>
                    <button
                      type="button"
                      ...
                      disabled={
                        yocoWebhookRegistering ||
                        !yocoCurrencyAllowed ||
                        !form.yocoCustomerSecretKeySet ||
                        yocoWebhookReady ||
                        yocoSetupNeedsSaveBeforeWebhook
                      }
                  ...
                  {!form.yocoCustomerSecretKeySet ? (
                    <p className="mt-3 rounded-2xl border border-amber-200 bg-amber-50 px-3 py-2 text-xs font-bold text-amber-900">
                      Save the Yoco secret key first, then come back and
                      register the webhook.
                    </p>
                  ) : null}
                  {yocoSetupNeedsSaveBeforeWebhook ? (
                    <p className="mt-3 rounded-2xl border border-amber-200 bg-amber-50 px-3 py-2 text-xs font-bold text-amber-900">
                      Save the selected Yoco mode/secret key before registering
                      the webhook.
                    </p>
                  ) : null}
                  {yocoWebhookReady ? (
                ...
                <div className="mt-4 rounded-2xl border border-slate-200 bg-white p-4">
                  <div className="flex flex-col gap-2 sm:flex-row sm:items-start sm:justify-between">
                    <div>
                      ...
                      <p className="mt-1 text-xs leading-5 text-slate-600">
                        Use this checklist before moving a tenant from Test to
                        Live. Live mode should use the tenant's Live Secret Key
                        and a webhook registered after the Live key is saved.
                      </p>
                    </div>
                    <span
                  ...
                  <div className="mt-3 grid gap-2 text-xs font-bold sm:grid-cols-2">
                    ...
                    >
                      ✓ Store currency:{" "}
                      {yocoCurrencyAllowed ? "ZAR" : "Change to ZAR"}
                    </p>
                    <p
                      className={
                        yocoCredentialReady
                    ...
                    >
                      ✓ Secret key: {yocoCredentialReady ? "saved" : "required"}
                    </p>
                    <p
                      className={
                    ...
                    >
                      ✓ Webhook:{" "}
                      {yocoWebhookReady ? "saved" : "register before live use"}
                    </p>
                    <p
                      className={
                  ...
                  {yocoLiveMode && !yocoWebhookReady ? (
                    <p className="mt-3 rounded-2xl border border-amber-200 bg-amber-50 px-3 py-2 text-xs font-bold text-amber-900">
                      Live mode should not be used without registering the Live
                      webhook first.
                    </p>
                  ) : null}
                </div>
                ...
                <label
                  className={`mt-4 flex items-start gap-3 rounded-2xl border p-3 text-sm ${yocoCurrencyAllowed && yocoCredentialReady ? "border-emerald-200 bg-white text-emerald-900" : "border-slate-200 bg-white text-slate-500"}`}
                >
                  <input
                    type="checkbox"
                    checked={form.enableYocoCustomerPayments}
                    onChange={(e) =>
                      update("enableYocoCustomerPayments", e.target.checked)
                    }
                    disabled={!yocoCurrencyAllowed || !yocoCredentialReady}
                    className="mt-1 h-5 w-5 rounded border-slate-300 text-emerald-600 focus:ring-emerald-200 disabled:opacity-50"
                  />
                  <span>
                    <strong>Enable Yoco setup for this tenant</strong>
                    <span className="mt-1 block text-xs leading-5">
                      Requires ZAR currency and a saved Yoco secret key. This
                      makes Yoco available to be switched on for checkout.
                    </span>
                  </span>
                </label>

                <label
                  className={`mt-3 flex items-start gap-3 rounded-2xl border p-3 text-sm ${form.enableYocoCustomerPayments && yocoCurrencyAllowed && yocoCredentialReady ? "border-emerald-200 bg-white text-emerald-900" : "border-slate-200 bg-white text-slate-500"}`}
                >
                  <input
                    type="checkbox"
                    checked={form.yocoCustomerPaymentsLive}
                    onChange={(e) =>
                      update("yocoCustomerPaymentsLive", e.target.checked)
                    }
                    disabled={
                      !form.enableYocoCustomerPayments ||
                      !yocoCurrencyAllowed ||
                  ...
                  />
                  <span>
                    <strong>Show Yoco on customer checkout</strong>
                    <span className="mt-1 block text-xs leading-5">
                      When enabled, ZAR storefront customers can choose Yoco and
                      will be sent to the hosted Yoco payment page. Live mode
                      requires a saved webhook first.
                    </span>
                  </span>
                </label>
              </div>
              ...
              <div className="space-y-4 text-sm">
                <div className="flex items-start justify-between gap-3">
                  <div>
                    ...
                    <p className="mt-1 text-xs leading-5 text-slate-600">
                      Hosted Pesapal checkout for Kenyan KES stores. Customers
                      will choose M-Pesa on the Pesapal payment page.
                    </p>
                  </div>
                  <span
                ...
                {!mpesaCurrencyAllowed ? (
                  <p className="mt-3 rounded-2xl border border-amber-200 bg-amber-50 px-3 py-2 text-xs font-bold text-amber-900">
                    M-Pesa/Pesapal is currently intended for Kenyan Shilling
                    stores. Change the store currency to KES before enabling it.
                  </p>
                ) : null}

                <div className="mt-3 rounded-2xl border border-red-200 bg-red-50 px-3 py-2 text-xs font-bold leading-5 text-red-900">
                  Important testing safety: Pesapal sandbox hosted checkout can
                  still trigger a real M-Pesa debit on a real Kenyan phone.
                  Ver-0.215A blocks sandbox hosted checkout by default. Use live
                  mode with the tenant's own Pesapal merchant account for
                  controlled low-value tests, or only enable sandbox hosted
                  checkout with the server environment flag after accepting that
                  risk.
                </div>
                ...
                <div className="mt-4 grid gap-4 md:grid-cols-2">
                  <Field label="Pesapal mode">
                    <select
                      value={form.mpesaCustomerMode}
                      onChange={(e) =>
                        update(
                          "mpesaCustomerMode",
                          e.target.value as FormState["mpesaCustomerMode"],
                        )
                      }
                      className="input"
                    >
                      <option value="test">
                        Test / sandbox (configuration only by default)
                      </option>
                  ...
                  <Field label="Pesapal account label">
                    <input
                      value={form.mpesaCustomerAccountLabel}
                      onChange={(e) =>
                        update("mpesaCustomerAccountLabel", e.target.value)
                      }
                      placeholder="Example: Nairobi Cafe Pesapal"
                      className="input"
                    />
                  </Field>
                  <Field label="Pesapal consumer key">
                    <input
                      value={form.mpesaCustomerConsumerKey}
                      onChange={(e) =>
                  ...
                  <Field
                    label={
                      form.mpesaCustomerConsumerSecretSet
                        ? `Pesapal consumer secret saved (${form.mpesaCustomerConsumerSecretHint || "saved"})`
                        : "Pesapal consumer secret"
                    }
                  >
                    <input
                      value={form.mpesaCustomerConsumerSecretInput}
                      onChange={(e) =>
                        update(
                          "mpesaCustomerConsumerSecretInput",
                          e.target.value,
                  ...
                  <Field label="Pesapal IPN notification ID">
                    <input
                      value={form.mpesaCustomerIpnId}
                      onChange={(e) =>
                        update("mpesaCustomerIpnId", e.target.value)
                      }
                      placeholder="Notification ID generated by Pesapal"
                      className="input"
                      autoComplete="off"
                    />
                  ...
                  <Field label="M-Pesa/Pesapal setup notes">
                    <input
                      value={form.mpesaCustomerSetupNotes}
                      onChange={(e) =>
                        update("mpesaCustomerSetupNotes", e.target.value)
                      }
                      placeholder="Example: Sandbox keys saved, waiting for test payment"
                      className="input"
                    />
                  </Field>
                </div>
                ...
                <div className="mt-4 rounded-2xl border border-slate-200 bg-white p-4">
                  <div className="flex flex-col gap-2 sm:flex-row sm:items-start sm:justify-between">
                    <div>
                      ...
                      <p className="mt-1 text-xs leading-5 text-slate-600">
                        Pesapal requires a consumer key, consumer secret and
                        registered IPN notification ID. Sandbox mode is now
                        treated as configuration-only unless the server
                        explicitly allows sandbox hosted checkout.
                      </p>
                    </div>
                    <span
                  ...
                  <div className="mt-3 grid gap-2 text-xs font-bold sm:grid-cols-2">
                    ...
                    >
                      ✓ Store currency:{" "}
                      {mpesaCurrencyAllowed ? "KES" : "Change to KES"}
                    </p>
                    <p
                      className={
                        form.mpesaCustomerConsumerKey.trim()
                    ...
                    >
                      ✓ IPN ID:{" "}
                      {form.mpesaCustomerIpnId.trim() ? "saved" : "required"}
                    </p>
                  </div>
                </div>
                  ...
                  <div className="flex flex-col gap-2 sm:flex-row sm:items-start sm:justify-between">
                    <div>
                      ...
                      <p className="mt-1 text-xs leading-5 text-slate-700">
                        Use this only for stuck M-Pesa/Pesapal attempts. It
                        checks Pesapal directly, blocks order creation unless
                        Pesapal returns COMPLETED, and can mark non-completed
                        attempts as failed for review.
                      </p>
                    </div>
                    <span className="inline-flex w-fit rounded-full border border-orange-200 bg-white px-3 py-1 text-[10px] font-black uppercase tracking-[0.14em] text-orange-900">
                      admin only
                    </span>
                  </div>
                  ...
                  <div className="mt-3 grid gap-3 lg:grid-cols-[1fr_auto_auto_auto] lg:items-end">
                    <Field label="Checkout ID / OrderTrackingId / merchant reference">
                      <input
                        ...
                        onChange={(event) =>
                          setMpesaDiagnosticReference(event.target.value)
                        }
                    ...
                    >
                      {mpesaDiagnosticChecking &&
                      mpesaDiagnosticAction === "check"
                        ? "Checking..."
                        : "Check status"}
                    </button>
                    <button
                      ...
                      disabled={
                        mpesaDiagnosticChecking ||
                        mpesaDiagnosticResult?.safeToCreateOrder !== true
                      }
                    ...
                    >
                      {mpesaDiagnosticChecking &&
                      mpesaDiagnosticAction === "create_order"
                        ? "Creating..."
                        : "Create order"}
                    </button>
                    <button
                    ...
                    >
                      {mpesaDiagnosticChecking &&
                      mpesaDiagnosticAction === "mark_failed"
                        ? "Updating..."
                        : "Mark failed"}
                    </button>
                  </div>
                    ...
                    >
                      <p className="text-sm font-black">
                        {mpesaDiagnosticResult.error
                          ? "Diagnostic error"
                          : mpesaDiagnosticResult.safeToCreateOrder
                            ? "Payment completed"
                            : "Payment not completed"}
                      </p>
                      <p className="mt-1 text-xs leading-5">
                        {mpesaDiagnosticResult.error ||
                          mpesaDiagnosticResult.message ||
                          "Status checked."}
                      </p>
                        ...
                        <div className="mt-3 grid gap-2 text-xs font-bold sm:grid-cols-2">
                          <p>
                            ...
                            <span className="font-mono font-semibold">
                              {mpesaDiagnosticResult.intent.id || "—"}
                            </span>
                          </p>
                          <p>
                            Status: {mpesaDiagnosticResult.intent.status || "—"}
                          </p>
                          <p>
                            Amount:{" "}
                            {mpesaDiagnosticResult.intent.currencyCode || ""}{" "}
                            {mpesaDiagnosticResult.intent.amountTotal ?? "—"}
                          </p>
                          <p>
                            Order:{" "}
                            {mpesaDiagnosticResult.intent.orderId ||
                              mpesaDiagnosticResult.orderId ||
                              "not created"}
                          </p>
                      ...
                      {mpesaDiagnosticResult.pesapal ? (
                        <div className="mt-3 rounded-2xl border border-white/70 bg-white/70 p-3 text-xs leading-5">
                          <p>
                            <strong>Pesapal status:</strong>{" "}
                            {mpesaDiagnosticResult.pesapal.status || "—"} / code{" "}
                            {String(
                              mpesaDiagnosticResult.pesapal.statusCode ?? "—",
                            )}
                          </p>
                          <p>
                            <strong>HTTP:</strong>{" "}
                            {mpesaDiagnosticResult.pesapal.httpStatus ?? "—"}
                          </p>
                          <p>
                            <strong>Confirmation:</strong>{" "}
                            {mpesaDiagnosticResult.pesapal.confirmationCode ||
                              "—"}
                          </p>
                          <p>
                            <strong>Method:</strong>{" "}
                            {mpesaDiagnosticResult.pesapal.paymentMethod || "—"}
                          </p>
                          {mpesaDiagnosticResult.pesapal.errorMessage ? (
                            <p>
                      ...
                      {mpesaDiagnosticResult.pesapal?.raw ? (
                        <details className="mt-3 rounded-2xl border border-white/70 bg-white/70 p-3 text-xs">
                          <summary className="cursor-pointer font-black">
                            Raw Pesapal response
                          </summary>
                          <pre className="mt-2 max-h-60 overflow-auto whitespace-pre-wrap break-words rounded-xl bg-slate-950 p-3 text-[11px] leading-5 text-white">
                            {JSON.stringify(
                              mpesaDiagnosticResult.pesapal.raw,
                              null,
                              2,
                ...
                <label
                  className={`mt-4 flex items-start gap-3 rounded-2xl border p-3 text-sm ${mpesaCurrencyAllowed && mpesaCredentialReady ? "border-emerald-200 bg-white text-emerald-900" : "border-slate-200 bg-white text-slate-500"}`}
                >
                  <input
                    type="checkbox"
                    checked={form.enableMpesaCustomerPayments}
                    onChange={(e) =>
                      update("enableMpesaCustomerPayments", e.target.checked)
                    }
                    disabled={!mpesaCurrencyAllowed || !mpesaCredentialReady}
                    className="mt-1 h-5 w-5 rounded border-slate-300 text-emerald-600 focus:ring-emerald-200 disabled:opacity-50"
                  />
                  <span>
                    <strong>Enable M-Pesa setup for this tenant</strong>
                    <span className="mt-1 block text-xs leading-5">
                      Requires KES currency, Pesapal credentials and an IPN
                      notification ID. This makes M-Pesa/Pesapal available to be
                      switched on for checkout.
                    </span>
                  </span>
                </label>

                <label
                  className={`mt-3 flex items-start gap-3 rounded-2xl border p-3 text-sm ${form.enableMpesaCustomerPayments && mpesaCurrencyAllowed && mpesaCredentialReady ? "border-emerald-200 bg-white text-emerald-900" : "border-slate-200 bg-white text-slate-500"}`}
                >
                  <input
                    type="checkbox"
                    checked={form.mpesaCustomerPaymentsLive}
                    onChange={(e) =>
                      update("mpesaCustomerPaymentsLive", e.target.checked)
                    }
                    disabled={
                      !form.enableMpesaCustomerPayments ||
                      !mpesaCurrencyAllowed ||
                  ...
                  />
                  <span>
                    <strong>Show M-Pesa on customer checkout</strong>
                    <span className="mt-1 block text-xs leading-5">
                      When enabled, KES storefront customers can choose M-Pesa
                      and will be sent to Pesapal's hosted payment page. In
                      test/sandbox mode the server will block checkout unless
                      sandbox hosted checkout has been deliberately allowed via
                      environment flag.
                    </span>
                  </span>
                </label>
            ...
            >
              <div className="space-y-4">
                <div className="flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between">
                  <div>
                    <p className="font-black text-slate-950">
                      Direct M-Pesa / Safaricom Daraja foundation
                    </p>
                    <p className="mt-1 text-xs leading-5 text-slate-600">
                      Store the tenant's Daraja credentials and run direct
                      Safaricom STK Push checkout. Ver-0.218 confirms successful
                      callbacks, creates the order, stores the M-Pesa receipt
                      and clears the cart.
                    </p>
                  </div>
                  <span
                ...
                {!darajaCurrencyAllowed ? (
                  <p className="mt-3 rounded-2xl border border-amber-200 bg-amber-50 px-3 py-2 text-xs font-bold text-amber-900">
                    Direct M-Pesa Daraja is prepared for Kenyan Shilling stores.
                    Change the store currency to KES before enabling the Daraja
                    setup.
                  </p>
                ) : null}
                ...
                <div className="mt-4 rounded-2xl border border-sky-100 bg-white/80 p-3 text-xs leading-5 text-sky-950">
                  ...
                  <p className="mt-1">
                    Customer enters phone number → Safaricom sends the M-Pesa
                    PIN prompt → Orduva records CheckoutRequestID → Safaricom
                    callback confirms payment → Orduva creates the order and
                    stores the M-Pesa receipt.
                  </p>
                </div>
                ...
                <div className="mt-3 rounded-2xl border border-emerald-200 bg-[#EAF3FA] p-3 text-xs leading-5 text-emerald-950">
                  ...
                  <p className="mt-1">
                    ...
                    has live Consumer Key, live Consumer Secret, live
                    shortcode/till/paybill, live Lipa Na M-Pesa Online passkey,
                    KES currency and the correct transaction type.
                  </p>
                </div>

                {darajaModeLive && darajaUsesSandboxShortcode ? (
                  <p className="mt-3 rounded-2xl border border-rose-200 bg-rose-50 px-3 py-2 text-xs font-bold text-rose-900">
                    Live mode cannot use the sandbox shortcode 174379. Replace
                    it with the tenant's live PayBill or Till number before
                    exposing Direct M-Pesa checkout.
                  </p>
                ) : null}

                {form.darajaPaymentsLive ? (
                  <p className="mt-3 rounded-2xl border border-emerald-200 bg-white px-3 py-2 text-xs font-bold text-emerald-900">
                    Direct M-Pesa is currently visible on customer checkout for
                    KES stores. Keep this on only after the live credentials and
                    shortcode have been confirmed.
                  </p>
                ) : null}
                ...
                <div className="mt-4 grid gap-4 md:grid-cols-2">
                  <Field label="Daraja mode">
                    <select
                      value={form.darajaCustomerMode}
                      onChange={(e) =>
                        update(
                          "darajaCustomerMode",
                          e.target.value as FormState["darajaCustomerMode"],
                        )
                      }
                      className="input"
                    >
                    ...
                    <p className="mt-2 text-xs leading-5 text-slate-500">
                      Use sandbox for Daraja sandbox tests. Use live only after
                      Safaricom has supplied/activated the tenant's production
                      app, shortcode/till/paybill and Lipa Na M-Pesa Online
                      passkey.
                    </p>
                  </Field>
                  <Field label="Daraja account label">
                    <input
                      value={form.darajaAccountLabel}
                      onChange={(e) =>
                        update("darajaAccountLabel", e.target.value)
                      }
                      placeholder="Example: ZimZa Safaricom Daraja"
                      className="input"
                    />
                  </Field>
                  <Field label="Daraja consumer key">
                    <input
                      value={form.darajaConsumerKey}
                      onChange={(e) =>
                        update("darajaConsumerKey", e.target.value)
                      }
                      placeholder="Consumer key from Safaricom Daraja"
                      className="input"
                      autoComplete="off"
                    />
                  </Field>
                  <Field
                    label={
                      form.darajaConsumerSecretSet
                        ? `Daraja consumer secret saved (${form.darajaConsumerSecretHint || "saved"})`
                        : "Daraja consumer secret"
                    }
                  >
                    <input
                      value={form.darajaConsumerSecretInput}
                      onChange={(e) =>
                        update("darajaConsumerSecretInput", e.target.value)
                      }
                      placeholder={
                        form.darajaConsumerSecretSet
                          ? "Leave blank to keep saved consumer secret"
                  ...
                  <Field label="Business shortcode / till / paybill">
                    <input
                      value={form.darajaShortcode}
                      onChange={(e) =>
                        update("darajaShortcode", e.target.value)
                      }
                      placeholder="Example: 174379 for sandbox, tenant live shortcode later"
                      className="input"
                      autoComplete="off"
                    />
                  </Field>
                  <Field
                    label={
                      form.darajaPasskeySet
                        ? `Daraja passkey saved (${form.darajaPasskeyHint || "saved"})`
                        : "Daraja passkey"
                    }
                  >
                    <input
                      value={form.darajaPasskeyInput}
                      onChange={(e) =>
                        update("darajaPasskeyInput", e.target.value)
                      }
                      placeholder={
                        form.darajaPasskeySet
                          ? "Leave blank to keep saved passkey"
                  ...
                  <Field label="Transaction type">
                    <select
                      value={form.darajaTransactionType}
                      onChange={(e) =>
                        update(
                          "darajaTransactionType",
                          e.target.value as FormState["darajaTransactionType"],
                        )
                      }
                      className="input"
                    >
                  ...
                  <Field label="Account reference prefix">
                    <input
                      value={form.darajaAccountReferencePrefix}
                      onChange={(e) =>
                        update(
                          "darajaAccountReferencePrefix",
                  ...
                  <div className="md:col-span-2">
                    <Field label="Future Daraja callback URL">
                      <input
                        value={form.darajaCallbackUrl}
                        onChange={(e) =>
                          update("darajaCallbackUrl", e.target.value)
                        }
                        placeholder="https://www.orduva.com/api/storefront/daraja/callback"
                        className="input"
                      />
                      <p className="mt-2 text-xs leading-5 text-slate-500">
                        Used as the STK Push callback URL. For Orduva live
                        stores this should normally be
                  ...
                  <div className="md:col-span-2">
                    <Field label="Daraja setup notes">
                      <input
                        value={form.darajaSetupNotes}
                        onChange={(e) =>
                          update("darajaSetupNotes", e.target.value)
                        }
                        placeholder="Example: Merchant account pending, waiting for live shortcode"
                        className="input"
                      />
                    </Field>
                  </div>
                ...
                <label
                  className={`mt-4 flex items-start gap-3 rounded-2xl border p-3 text-sm ${darajaCurrencyAllowed && darajaCredentialReady ? "border-emerald-200 bg-white text-emerald-900" : "border-slate-200 bg-white text-slate-500"}`}
                >
                  <input
                    type="checkbox"
                    checked={form.enableDarajaCustomerPayments}
                    onChange={(e) =>
                      update("enableDarajaCustomerPayments", e.target.checked)
                    }
                    disabled={
                      !darajaLiveReadinessOk &&
                      !form.enableDarajaCustomerPayments
                  ...
                  <span>
                    <strong>
                      Enable direct M-Pesa Daraja setup for this tenant
                    </strong>
                    <span className="mt-1 block text-xs leading-5">
                      Requires KES currency, Daraja consumer key, consumer
                      secret, shortcode and passkey. Live mode also requires a
                      real tenant shortcode/till/paybill, not the sandbox
                      shortcode.
                    </span>
                  </span>
                </label>
                ...
                <label
                  className={`mt-3 flex items-start gap-3 rounded-2xl border p-3 text-sm ${form.enableDarajaCustomerPayments && darajaCurrencyAllowed && darajaCredentialReady ? "border-emerald-200 bg-white text-emerald-900" : "border-slate-200 bg-slate-100 text-slate-500"}`}
                >
                  <input
                    type="checkbox"
                    checked={form.darajaPaymentsLive}
                    onChange={(e) =>
                      update("darajaPaymentsLive", e.target.checked)
                    }
                    disabled={
                      !form.enableDarajaCustomerPayments ||
                      (!darajaLiveReadinessOk && !form.darajaPaymentsLive)
                    }
                    className="mt-1 h-5 w-5 rounded border-slate-300 text-emerald-600 focus:ring-emerald-200 disabled:opacity-50"
                  />
                  <span>
                    <strong>Show direct M-Pesa on customer checkout</strong>
                    <span className="mt-1 block text-xs leading-5">
                      When enabled, KES customers can choose direct M-Pesa,
                      receive a Safaricom STK Push prompt, and Orduva will
                      create the order only after Safaricom confirms successful
                      payment.
                    </span>
                  </span>
                </label>
        ...
        <Section
          ...
          title="Advanced currency display"
          dirty={currencyDirty}
          saving={saving}
        >
          <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-3">
            ...
            <Field label="Suffix">
              <input
                value={form.currencySuffix}
                onChange={(e) => update("currencySuffix", e.target.value)}
                className="input"
                maxLength={12}
            ...
            <Field label="Display mode">
              <select
                ...
                onChange={(e) =>
                  update(
                    "currencyDisplayMode",
                    e.target.value as FormState["currencyDisplayMode"],
                  )
                }
                className="input"
              >
            ...
            <Field label="Prefix position">
              <select
                ...
                onChange={(e) =>
                  update(
                    "currencySymbolPosition",
                    e.target.value as FormState["currencySymbolPosition"],
                  )
                }
                className="input"
              >
            ...
            <Field label="Decimal places">
              <input
                ...
                onChange={(e) =>
                  update("currencyDecimalPlaces", e.target.value)
                }
                className="input"
              />
            </Field>
          ...
          <label className="mt-4 flex items-center gap-3 rounded-2xl border border-slate-200 bg-white px-4 py-3 text-sm text-slate-700">
            <input
              type="checkbox"
              checked={form.currencyUseThousandsSeparator}
              onChange={(e) =>
                update("currencyUseThousandsSeparator", e.target.checked)
              }
          ...
          <p className="mt-3 text-xs leading-5 text-slate-500">
            Currency suffix is optional. Leave it blank for normal pricing, or
            enter a tenant-specific suffix such as /- only when that store needs
            it.
          </p>
            ...
            {[100, 295, 1000].map((amount) => (
              ...
              >
                ...
                <p className="mt-2 text-xl font-semibold text-slate-950">
                  {formatMoney(amount, moneySettings)}
                </p>
              </div>
            ))}
          </div>
        ...
        {message ? (
          <div
            className={`mt-5 rounded-2xl border px-4 py-3 text-sm ${messageClass}`}
          >
            {message}
          </div>
        ) : null}
  ...
  return (
    <div
      ...
      style={{
        ...
        borderColor: normalizeThemeColor(theme.globalBorder, "#D9C7A3"),
        color: text,
      }}
    >
      {target === "global" ? (
        ...
        >
          ...
          <p
            ...
            style={{
              color: normalizeThemeColor(theme.globalSoftText, "#64748B"),
            }}
          >
            This shows the page background, main text and soft text treatment.
          </p>
        </div>
      ) : null}
      {target === "header" ? (
        <div
          ...
          style={{
            ...
            borderColor: normalizeThemeColor(
              theme.headerButtonBorder,
              "#D9C7A3",
            ),
            color: normalizeThemeColor(theme.headerText, "#2B2B2B"),
          }}
        >
          <div className="flex items-center justify-between gap-3">
            <div className="flex min-w-0 items-center gap-3">
              {logoUrl ? (
                <img
                  src={logoUrl}
                  alt="Logo preview"
                  className="max-h-11 max-w-[145px] shrink-0 object-contain"
                />
              ) : null}
              <strong className="truncate">{previewName}</strong>
            </div>
          ...
          {faviconUrl ? (
            <div className="mt-3 flex items-center gap-2 rounded-xl border border-black/5 bg-white/75 px-3 py-2 text-[11px] opacity-80">
              <img
                src={faviconUrl}
                alt="Favicon preview"
                className="h-5 w-5 object-contain"
              />
              <span>Favicon saved for browser tab / app icon preview</span>
            </div>
          ) : null}
        </div>
      ) : null}
      {target === "welcome" ? (
        <div
          ...
          style={{
            ...
            borderColor: normalizeThemeColor(theme.welcomeBorder, "#D9C7A3"),
            boxShadow: `0 10px 24px ${normalizeThemeColor(theme.welcomeShadow, "#D9C7A3")}18`,
          }}
        >
          {logoUrl ? (
            <img
          ...
          <p
            ...
            style={{
              color: normalizeThemeColor(theme.welcomeLabel, "#C7922F"),
            }}
          >
            Welcome
          </p>
          <h4
            ...
            style={{
              color: normalizeThemeColor(theme.welcomeHeading, "#0F172A"),
            }}
          >
            {previewHeading}
          </h4>
          <p
            ...
            style={{ color: normalizeThemeColor(theme.welcomeBody, "#2B2B2B") }}
          >
            {previewSubheading}
          </p>
          <div className="mt-4 flex flex-col items-center gap-2">
            <div
              className="flex min-h-12 w-full max-w-[220px] items-center justify-center gap-2 rounded-2xl bg-white/90 px-3 py-2 text-xs font-black shadow-sm"
              style={{
                color: normalizeThemeColor(theme.welcomeActionText, "#0F172A"),
                borderColor: normalizeThemeColor(
                  theme.welcomeActionBorder,
                  "#D9C7A3",
                ),
              }}
            >
              <span
                ...
                style={{
                  backgroundColor: normalizeThemeColor(
                    theme.welcomeActionIconBackground,
                    "#10B981",
                  ),
                  color: normalizeThemeColor(
                    theme.welcomeActionIconText,
                    "#FFFFFF",
                  ),
                }}
              >
            ...
            <div className="grid w-full max-w-[240px] grid-cols-3 gap-2 text-center text-[10px] font-bold uppercase">
              {["Offers", "Favourites", "Buy again"].map((label) => (
                <span
                  key={label}
                  className="rounded-2xl bg-white/85 px-2 py-2 shadow-sm"
                  style={{
                    color: normalizeThemeColor(
                      theme.welcomeActionText,
                      "#0F172A",
                    ),
                  }}
                >
          ...
          <div className="mt-4 grid gap-3 sm:grid-cols-2">
            ...
            >
              <div
                ...
                style={{
                  backgroundColor: normalizeThemeColor(
                    theme.rewardsPopupHeaderBackground,
                    "#334155",
                  ),
                  color: normalizeThemeColor(
            ...
            <div>
              <div className="flex items-start justify-between gap-3">
                <h4
                  className="font-bold"
                  style={{
                    color: normalizeThemeColor(theme.productTitle, "#0F172A"),
                  }}
                >
                  Sample product
                </h4>
              ...
              <div className="mt-4 grid grid-cols-3 gap-2">
                <span
                  ...
                  style={{
                    backgroundColor: normalizeThemeColor(
                      theme.priceBoxBackground,
                      "#FFFFFF",
                    ),
                    borderColor: normalizeThemeColor(
                      theme.priceBoxBorder,
                      "#D9C7A3",
                    ),
                    color: normalizeThemeColor(theme.priceText, "#0F172A"),
                  }}
                >
                  {money}
                </span>
                <span
                  ...
                  style={{
                    ...
                    color: normalizeThemeColor(theme.addButtonText, "#0F172A"),
                  }}
                >
                  Add
                </span>
                <span
                  ...
                  style={{
                    ...
                    color: normalizeThemeColor(theme.moreButtonText, "#0F172A"),
                  }}
                >
                  More
                </span>
      ...
      {target === "favourites" ? (
        <div
          ...
          style={{
            ...
            borderColor: normalizeThemeColor(theme.favouritesBorder, "#F59E0B"),
            color: normalizeThemeColor(theme.favouritesText, "#FFFFFF"),
          }}
        >
          <p
            className="text-[10px] font-black uppercase tracking-[0.22em]"
            style={{
              color: normalizeThemeColor(theme.favouritesLabelText, "#FDE68A"),
            }}
          >
            Your favourites
          </p>
          <h4 className="mt-1 text-lg font-black">Saved favourites</h4>
          <div
            className="mx-auto mt-3 max-w-[220px] rounded-[20px] border p-3 text-center"
            style={{
              backgroundColor: normalizeThemeColor(
                theme.favouritesCardBackground,
                "#FFFFFF",
              ...
              borderColor: normalizeThemeColor(
                theme.favouritesCardBorder,
                "#FCD34D",
              ),
              boxShadow:
                theme.favouritesCardShadowEnabled === false
                  ? "none"
                  : `0 8px 18px ${normalizeThemeColor(theme.favouritesCardShadow, "#F59E0B")}14`,
            }}
          >
            <div className="mx-auto aspect-[1.25/1] rounded-2xl bg-slate-100" />
            <h5
              className="mt-3 font-black"
              style={{
                color: normalizeThemeColor(
                  theme.favouritesCardTitle,
                  "#0F172A",
                ),
              }}
            >
            ...
            <div className="mt-3 flex items-center justify-center gap-2">
              <span
                className="rounded-xl border px-2.5 py-1.5 text-xs font-black"
                style={{
                  backgroundColor: normalizeThemeColor(
                    theme.favouritesPriceBackground,
                    "#FFFFFF",
                  ),
                  borderColor: normalizeThemeColor(
                    theme.favouritesPriceBorder,
                    "#F59E0B",
                  ),
                  color: normalizeThemeColor(
                    theme.favouritesPriceText,
                    "#0F172A",
                  ),
                }}
              >
              ...
              <span
                ...
                style={{
                  ...
                  borderColor: normalizeThemeColor(
                    theme.favouritesAddBorder,
                    "#F59E0B",
                  ),
                  color: normalizeThemeColor(
                    theme.favouritesAddText,
                    "#FFFFFF",
                  ),
                }}
              >
      ...
      {target === "footer" ? (
        <div
          ...
          style={{
            ...
            borderColor: normalizeThemeColor(theme.globalBorder, "#D9C7A3"),
            color: normalizeThemeColor(theme.footerText, "#2B2B2B"),
          }}
        >
          <p className="text-[11px] font-bold uppercase tracking-[0.18em]">
            Storefront footer
          </p>
          <p className="mt-2 text-sm leading-5">{footerBlurb}</p>
          <p className="mt-2 text-xs leading-5">{footerNotice}</p>
          <div className="mt-3 flex flex-wrap items-center gap-2">
            <span
              className="inline-flex rounded-full px-3 py-1.5 text-xs font-bold text-white"
              style={{
                backgroundColor: normalizeThemeColor(
                  theme.footerBadgeBackground,
                  "#C7922F",
            ...
            <span className="inline-flex rounded-full border border-black/10 bg-white/75 px-3 py-1.5 text-xs font-bold">
              Social links
            </span>
          </div>
        </div>
...
function ToggleRow({
  ...
  return (
    <label className="flex cursor-pointer items-center justify-between gap-4 rounded-2xl border border-slate-100 bg-slate-50/70 p-3">
      ...
      <span
        className={`relative inline-flex h-7 w-12 shrink-0 items-center rounded-full border transition ${checked ? "border-orange-300 bg-orange-500" : "border-slate-300 bg-slate-200"}`}
      >
        <input
          type="checkbox"
          checked={checked}
        ...
        />
        <span
          className={`inline-block h-5 w-5 rounded-full bg-white shadow-sm transition ${checked ? "translate-x-5" : "translate-x-1"}`}
          aria-hidden="true"
        />
      </span>
    </label>
  ...
  return (
    <div className="rounded-2xl border border-slate-100 bg-slate-50/70 p-3">
      <div className="mb-2 flex items-center justify-between gap-3">
        ...
        <label
          className="inline-flex h-9 w-9 shrink-0 cursor-pointer items-center justify-center rounded-xl border border-slate-200 bg-white text-base shadow-sm transition hover:border-orange-300 hover:bg-orange-50 sm:hidden"
...
function ReadOnlyAssetUrl({ label, value }: { label: string; value: string }) {
  return (
    <div className="rounded-2xl border border-slate-100 bg-slate-50 p-3">
      ...
      <p className="mt-1 break-all text-[11px] leading-5 text-slate-600">
        {value}
      </p>
    </div>
  );
        ...
        <div>
          ...
          {saved ? (
            <p className="mt-1 text-xs font-semibold text-emerald-700">
              Current file saved. Upload a new file only if you want to change
              it.
            </p>
          ) : null}
        </div>
...
const SETTINGS_MENU_ITEMS = [
  ...
  },
  {
    ...
    group: "Brand",
    title: "Logo and favicon",
    help: "Upload or replace the store logo and browser icon.",
  },
  {
    id: "branding-and-wording",
    group: "Brand",
    title: "Branding and wording",
    help: "Business name, storefront heading and admin labels.",
  },
  {
    id: "theme-presets",
    group: "Theme",
    title: "Theme presets",
    help: "Choose a ready-made colour starting point.",
  },
  {
    id: "storefront-additions",
    group: "Theme",
    title: "Storefront welcome & About us",
    help: "Add a welcome banner and an About us section before products.",
  },
  {
    id: "per-item-storefront-colours",
    group: "Theme",
    title: "Per-item storefront colours",
    help: "Fine-tune each visible storefront area.",
  },
  {
    id: "business-contact-details",
    group: "Contact",
  ...
  },
  {
    ...
    group: "Payments",
    title: "Invoice payments",
    help: "Enable a dedicated first storefront section for invoice, deposit and statement balance payment cards.",
  },
  {
    id: "receipt-information",
    group: "Receipts",
    title: "Receipt information",
    help: "Document name, tax details, optional fields, receipt image and footer wording.",
  },
  {
    id: "storefront-seo",
    group: "SEO",
  ...
  },
  {
    ...
    group: "Rewards",
    title: "Customer rewards programme",
    help: "Silver, Gold and Platinum spend tiers with percentage discounts.",
  },
  {
    id: "discounts-and-codes",
    group: "Discounts",
    title: "Discounts & codes",
    help: "Product, combo and site-wide promotional offers.",
  },
  {
    id: "storefront-payment-options",
    group: "Payments",
  ...
  },
  {
    ...
    group: "Payments",
    title: "Advanced currency display",
    help: "Currency name, symbol, suffix, separators and sample pricing.",
  },
];

const SETTINGS_SECTION_META: Record<
  string,
  { group: string; help: string; accent: string }
> = {
  "admin-workspace": {
    group: "Workspace",
  ...
  "storefront-additions": {
    group: "Theme",
    help: "Welcome banner, overlay and About us content for the storefront.",
    accent: "border border-slate-200 bg-slate-50 text-slate-700",
  },
      ...
      >
        <div className="orduva-admin-popup-header">
          <div className="flex items-start justify-between gap-4">
            <div className="min-w-0">
              ...
              <p className="mt-2 text-sm leading-6 text-slate-600">
                Use this when helping a non-technical store owner connect their
                own Stripe account for storefront customer payments.
              </p>
            </div>
            <button
              type="button"
        ...
        <div className="orduva-admin-popup-body">
          <div className="rounded-[22px] border border-orange-200 bg-orange-50 p-4 text-sm leading-6 text-orange-950">
            ...
            customer order money should go to the store owner, not the Orduva
          ...
          <div className="mt-4 grid gap-3 md:grid-cols-3">
            <div className="rounded-2xl border border-slate-200 bg-white p-4">
              ...
              <p className="mt-2 font-mono text-sm font-black text-slate-950">
                pk_test_...
              </p>
              <p className="mt-1 text-xs leading-5 text-slate-500">
                Safe for browser-side Stripe setup.
              </p>
            </div>
            <div className="rounded-2xl border border-slate-200 bg-white p-4">
              <p className="text-[11px] font-black uppercase tracking-[0.16em] text-slate-500">
              ...
              <p className="mt-1 text-xs leading-5 text-slate-500">
                Private. Store server-side only.
              </p>
            </div>
            <div className="rounded-2xl border border-slate-200 bg-white p-4">
              <p className="text-[11px] font-black uppercase tracking-[0.16em] text-slate-500">
              ...
              <p className="mt-1 text-xs leading-5 text-slate-500">
                Used to verify Stripe payment messages.
              </p>
            </div>
          </div>
          ...
          <div className="mt-4 rounded-[22px] border border-amber-200 bg-amber-50 p-4 text-xs leading-5 text-amber-950">
            ...
            <span className="mt-2 block">
              ...
              https://www.orduva.com/api/billing/stripe/webhook — that endpoint
              is only for Orduva subscription billing.
            </span>
          </div>

          <div className="mt-4 rounded-[22px] border border-indigo-200 bg-indigo-50 p-4 text-xs leading-5 text-indigo-950">
            <p className="font-black uppercase tracking-[0.16em] text-indigo-700">
              Required Stripe webhook events
            </p>
            <p className="mt-2">
              Use destination name <strong>Orduva - Customer Orders</strong> and
              select these events:
            </p>
            <ul className="mt-2 list-disc space-y-1 pl-5">
              ...
              <li>
                <span className="font-mono">checkout.session.expired</span>
              </li>
              <li>
                <span className="font-mono">payment_intent.succeeded</span>
              </li>
              <li>
                <span className="font-mono">payment_intent.payment_failed</span>
              </li>
              <li>
    ...
    <div className={`rounded-[22px] border p-4 ${toneClass}`}>
      <div className="flex items-center justify-between gap-3">
        <span className="text-lg" aria-hidden="true">
          {tone === "platinum" ? "✦" : tone === "gold" ? "★" : "◇"}
        </span>
        <span className="rounded-full border border-white/40 bg-white/20 px-2.5 py-1 text-[10px] font-black uppercase tracking-[0.14em]">
          {discount || "0"}% off
        </span>
      </div>
      <p className="mt-3 text-base font-black">{name}</p>
      <p className="mt-1 text-xs leading-5 opacity-80">
        Spend {spend} to qualify.
      </p>
    </div>
  );
}
  ...
  return (
    <details className="group mx-auto w-full overflow-hidden rounded-[22px] border border-slate-200 bg-white shadow-none transition open:border-slate-300 md:hover:border-orange-300 md:hover:ring-2 md:hover:ring-orange-100">
      <summary className="flex cursor-pointer list-none items-start justify-between gap-3 px-3 py-3 text-left outline-none transition focus-visible:ring-2 focus-visible:ring-orange-200 sm:px-4 sm:py-4 [&::-webkit-details-marker]:hidden">
        <span className="min-w-0">
          <span className="mb-2 inline-flex rounded-full bg-slate-100 px-2.5 py-1 text-[10px] font-black uppercase tracking-[0.14em] text-slate-600">
            Payment gateway
          </span>
        ...
        <span className="flex shrink-0 flex-col items-end gap-2">
          ...
          <span
            className="inline-flex h-8 w-8 items-center justify-center rounded-full border border-slate-200 bg-white text-base font-black text-slate-700 transition group-open:rotate-45 group-open:border-orange-300 group-open:text-orange-800"
            aria-hidden="true"
          >
            +
          </span>
      ...
      <div className="px-3 pb-3 pt-0 sm:px-4 sm:pb-4">{children}</div>
    </details>
  );
}
