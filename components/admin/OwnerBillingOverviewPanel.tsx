...
function revenueLine(totals: CurrencyTotal[]) {
  ...
  return totals
    .map(
      (item) => `${money(item.amount, item.currencyCode)} (${item.payments})`,
    )
    .join(" · ");
}
...
function isExpiredTrial(store: BillingStore) {
  ...
  return (
    ...
    (state === "trial" && trialDays !== null && trialDays < 0)
  );
}
...
function matchesOwnerView(store: BillingStore, view: OwnerClientView) {
  ...
  if (view === "trial")
    return store.billingState === "trial" && !isExpiredTrial(store);
...
function viewHelp(view: OwnerClientView) {
  if (view === "paid")
    return "Stores with active billing. Keep this list clean and easy to scan.";
  if (view === "trial")
    return "Stores still in their free trial. Follow up before the trial ends.";
...
function statusPillClasses(view: OwnerClientView) {
  ...
  if (view === "trial")
    return "border-[#336699]/35 bg-[#EAF3FB] text-[#28547D]";
...
function detailCard(label: string, value: string) {
  return (
    <div className="rounded-2xl border border-[#0E0E10]/10 bg-white px-4 py-3">
      <p className="text-[10px] font-semibold uppercase tracking-[0.16em] text-[#8B929C]">
        {label}
      </p>
...
export default function OwnerBillingOverviewPanel() {
  ...
  const [ownerAssistBusyId, setOwnerAssistBusyId] = useState<string | null>(
    null,
  );
  const [extendBusyKey, setExtendBusyKey] = useState<string | null>(null);
  ...
  const loadBillingOverview = useCallback(async () => {
    ...
    try {
      ...
      const data = (await response
        .json()
        .catch(() => ({}))) as BillingOverviewPayload;
      if (!response.ok)
        throw new Error(
          data?.error || "Could not load owner platform overview.",
        );
      setPayload(data);
      setMessage("");
    } catch (error) {
      setMessage(
        error instanceof Error
          ? error.message
          : "Could not load owner platform overview.",
      );
    } finally {
      setLoading(false);
    }
  ...
  const paidStores = useMemo(
    () => data.stores.filter((store) => matchesOwnerView(store, "paid")),
    [data.stores],
  );
  const trialStores = useMemo(
    () => data.stores.filter((store) => matchesOwnerView(store, "trial")),
    [data.stores],
  );
  const expiredStores = useMemo(
    () => data.stores.filter((store) => matchesOwnerView(store, "expired")),
    [data.stores],
  );
  ...
  const activeStores = useMemo(
    () =>
      data.stores.filter(
        (store) =>
          matchesOwnerView(store, activeView) &&
  ...
  const cards: Array<{
  ...
  }> = [
    {
      ...
      hint: "Active billing",
      classes: "border-[#336699]/35 bg-[#EAF3FB] text-[#28547D]",
    },
    {
      key: "trial",
      label: "Free trial",
      value: trialStores.length,
      hint: "Still deciding",
      classes: "border-[#336699]/35 bg-white text-[#28547D]",
    },
    {
      key: "expired",
      label: "Trial expired",
      value: expiredStores.length,
      hint: "Follow up",
  ...
  const paymentAttentionCount =
    data.summary.paymentAttention + data.summary.missingStripeLink;
  ...
  const exportCurrentView = useCallback(() => {
    downloadCsv(
      ...
      activeStores.map((store) => [
        ...
        store.trialEndsAt || "",
        store.lastPayment
          ? money(store.lastPayment.amount, store.lastPayment.currencyCode)
          : "",
  ...
  const extendTrial = useCallback(
    async (store: BillingStore, additionalDays: number) => {
      if (!store.id) return;
      const safeDays = Math.max(
        1,
        Math.min(365, Math.floor(Number(additionalDays) || 0)),
      );
      setExtendBusyKey(`${store.id}:${safeDays}`);
      setMessage(
        `Extending ${store.name} by ${safeDays} day${safeDays === 1 ? "" : "s"}...`,
      );
      try {
        ...
        setMessage(
          `Trial extended by ${safeDays} day${safeDays === 1 ? "" : "s"} for ${store.name}.`,
        );
        ...
        setExtendBusyKey(null);
      }
    },
    [loadBillingOverview, ownerAccess.platformHeaders],
  );

  const openOwnerAssistAdmin = useCallback(
    async (store: BillingStore) => {
      ...
      try {
        ...
        if (!response.ok)
          throw new Error(data?.error || "Could not open tenant admin.");
        setMessage(
          `Owner support access ready for ${data?.tenant?.name || store.name}.`,
        );
    ...
    <section className="overflow-hidden rounded-[30px] border border-[#0E0E10]/10 bg-white shadow-[0_18px_50px_rgba(14,14,16,0.08)]">
      <div className="bg-gradient-to-br from-[#0E0E10] via-[#102338] to-[#336699] p-5 text-white sm:p-6">
        <div className="flex flex-col gap-4 lg:flex-row lg:items-start lg:justify-between">
          <div>
            <p className="text-xs font-semibold uppercase tracking-[0.22em] text-[#BFD8EE]">
              Owner platform
            </p>
            ...
            <p className="mt-3 max-w-3xl text-sm leading-6 text-white/72">
              A simpler owner view. Start with the three important groups, then
              open one client row only when you need the account details.
            </p>
          </div>
        ...
        <div className="mt-5 grid gap-3 sm:grid-cols-3">
          <div className="rounded-[22px] border border-white/15 bg-white/10 p-4">
            <p className="text-[11px] font-semibold uppercase tracking-[0.16em] text-[#BFD8EE]">
              Total stores
            </p>
            ...
            <p className="mt-2 text-xs font-bold text-white/58">
              Across all owner platform records.
            </p>
          </div>
          <div className="rounded-[22px] border border-[#339933]/35 bg-[#339933]/14 p-4">
            <p className="text-[11px] font-semibold uppercase tracking-[0.16em] text-[#DDFBE4]">
              Last 30 days
            </p>
            <p className="mt-2 text-xl font-semibold leading-tight text-white">
              {revenueLine(data.summary.last30DaysRevenue)}
            </p>
            <p className="mt-2 text-xs font-bold text-white/58">
              Recorded Stripe billing payments.
            </p>
          </div>
          <div className="rounded-[22px] border border-[#336699]/35 bg-white/10 p-4">
            ...
            <p className="mt-2 text-xs font-bold text-white/58">
              Payment attention or missing Stripe links.
            </p>
          </div>
        </div>
      </div>
      ...
      <div className="border-t border-[#0E0E10]/10 p-5 sm:p-6">
        {message ? (
          <p className="mb-4 rounded-2xl border border-[#336699]/20 bg-[#F3F7FA] px-4 py-3 text-sm font-bold text-[#336699]">
            {message}
          </p>
        ) : null}
        ...
        <div className="mt-5 rounded-[28px] border border-[#0E0E10]/10 bg-[#F3F7FA] p-4 sm:p-5">
          <div className="flex flex-col gap-3 lg:flex-row lg:items-end lg:justify-between">
            <div>
              <p className="text-xs font-semibold uppercase tracking-[0.18em] text-[#336699]">
                Selected list
              </p>
              <h3 className="mt-1 text-2xl font-semibold text-[#0E0E10]">
            ...
            <div className="rounded-2xl border border-[#0E0E10]/10 bg-white p-3 lg:min-w-[280px]">
              <label
                className="text-[11px] font-semibold uppercase tracking-[0.16em] text-[#8B929C]"
                htmlFor="owner-client-search"
              >
                Search this list
              </label>
              <input
          ...
          <div className="mt-4 rounded-[22px] border border-[#0E0E10]/10 bg-white">
            <div className="hidden grid-cols-[1.4fr_0.8fr_0.8fr_0.9fr_0.35fr] gap-3 rounded-t-[22px] border-b border-[#0E0E10]/10 bg-[#F8FAFC] px-4 py-3 text-[11px] font-semibold uppercase tracking-[0.14em] text-[#8B929C] lg:grid">
              ...
              <span>Plan</span>
              <span>
                {activeView === "paid" ? "Last payment" : "Trial ends"}
              </span>
            ...
            {activeStores.length ? (
              <div className="space-y-3 p-3">
                {activeStores.map((store) => {
                  const storeUrl = store.slug
                    ? `https://${store.slug}.orduva.com`
                    : "#";
                  const expanded = expandedStoreId === store.id;
                  return (
                    <article
                      key={store.id}
                      className="overflow-hidden rounded-[20px] border border-[#0E0E10]/10 bg-white text-sm shadow-[0_10px_28px_rgba(14,14,16,0.04)]"
                    >
                      <button
                        type="button"
                        onClick={() =>
                      ...
                      >
                        <div>
                          ...
                          <p className="mt-1 text-xs font-bold text-[#336699]">
                            {store.slug
                              ? `${store.slug}.orduva.com`
                              : "No slug"}
                          </p>
                        </div>
                        <div>
                          <span
                            className={`inline-flex rounded-full border px-3 py-1 text-[11px] font-semibold uppercase tracking-[0.1em] ${statusPillClasses(activeView)}`}
                          >
                        ...
                        <span
                          className="mx-auto inline-flex min-h-10 items-center justify-center rounded-2xl border border-[#0E0E10]/10 bg-white px-3 text-xs font-semibold text-[#336699] shadow-sm transition hover:border-[#336699]/40 hover:bg-[#EAF3FB]"
                          aria-label={
                      ...
                      {expanded ? (
                        <div className="border-t border-[#0E0E10]/8 bg-[#FBFCFD] px-4 py-5">
                          <div className="flex flex-col gap-4 lg:flex-row lg:items-start lg:justify-between">
                            <div>
                              <p className="text-[11px] font-semibold uppercase tracking-[0.18em] text-[#8B929C]">
                                Account information
                              </p>
                              <h4 className="mt-1 text-lg font-semibold text-[#0E0E10]">
                                {store.name}
                              </h4>
                              <p className="mt-2 max-w-2xl text-sm font-semibold leading-6 text-[#68707A]">
                                Owner-only billing and access details. Checklist
                                progress is kept out of this view so the client
                                list stays focused.
                              </p>
                            </div>
                            <div className="flex flex-wrap gap-2">
                          ...
                          {activeView === "expired" ? (
                            <div className="mt-4 rounded-[24px] border border-[#FF6A3D]/25 bg-[#FFF7F0] p-4">
                              <div className="flex flex-col gap-3 lg:flex-row lg:items-center lg:justify-between">
                                <div>
                                  <p className="text-[11px] font-semibold uppercase tracking-[0.18em] text-[#C84F2A]">
                                    Extend trial
                                  </p>
                                  <p className="mt-1 text-sm font-bold leading-6 text-[#374151]">
                                    Restore this expired store by adding time to
                                    the trial.
                                  </p>
                                </div>
                                <div className="flex flex-wrap gap-2">
                                  {[1, 7, 14, 30].map((days) => {
                                    const busy =
                                      extendBusyKey === `${store.id}:${days}`;
                                    return (
                                      <button
                                        key={days}
                                        type="button"
                                        onClick={() =>
                                          void extendTrial(store, days)
                                        }
                                        disabled={Boolean(extendBusyKey)}
                                        className="inline-flex min-h-10 items-center justify-center rounded-2xl bg-[#336699] px-4 py-2 text-xs font-black text-white transition hover:bg-[#28547D] disabled:cursor-not-allowed disabled:opacity-60"
                                      >
                                        {busy
                                          ? "Extending..."
                                          : `+${days} day${days === 1 ? "" : "s"}`}
                          ...
                          <div className="mt-4 grid gap-3 sm:grid-cols-2 xl:grid-cols-4">
                            ...
                            {detailCard(
                              "Billing state",
                              store.billingState || "Not recorded",
                            )}
                            {detailCard(
                              "Billing provider",
                              store.billingProvider || "Not recorded",
                            )}
                            {detailCard(
                              "Subscription",
                              store.subscriptionStatus || "Not recorded",
                            )}
                            {detailCard(
                              "Trial status",
                              store.trialStatus || "Not recorded",
                            )}
                            {detailCard(
                              "Trial ends",
                              formatDateTime(store.trialEndsAt),
                            )}
                            {detailCard(
                              "Plan",
                              store.planLabel || "Not recorded",
                            )}
                            {detailCard(
                              "Stripe customer",
                              store.stripeCustomerId || "Not linked",
                            )}
                            {detailCard(
                              "Stripe subscription",
                              store.stripeSubscriptionId || "Not linked",
                            )}
                            {detailCard(
                              "Last payment",
                              store.lastPayment
                            ...
                            {detailCard(
                              "Last payment date",
                              formatDateTime(store.lastPayment?.paidAt),
                            )}
                          </div>
                        </div>
                      ) : null}
              ...
              <div className="px-4 py-8 text-center text-sm font-bold text-[#68707A]">
