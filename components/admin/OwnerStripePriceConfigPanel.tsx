    ...
    <section className="space-y-5">
      <div className="rounded-[30px] border border-[#0E0E10]/10 bg-white p-5 shadow-[0_18px_54px_rgba(14,14,16,0.08)] sm:p-6">
        <div className="flex flex-col gap-4 lg:flex-row lg:items-center lg:justify-between">
          <div>
            <p className="text-xs font-semibold uppercase tracking-[0.22em] text-[#336699]">Configuration status</p>
            <h2 className="mt-2 text-2xl font-semibold tracking-tight text-[#0E0E10]">Stripe Price IDs</h2>
        ...
        <div className="mt-5 grid gap-3 sm:grid-cols-3">
          <div className="rounded-2xl border border-[#0E0E10]/10 bg-[#F3F7FA] p-4">
            <p className="text-xs font-semibold uppercase tracking-[0.16em] text-[#5C5F66]">Configured</p>
            <p className="mt-1 text-3xl font-semibold text-[#0E0E10]">{data?.configuredCount ?? 0}/{data?.total ?? 30}</p>
          </div>
          <div className="rounded-2xl border border-[#336699]/20 bg-[#336699]/10 p-4">
            <p className="text-xs font-semibold uppercase tracking-[0.16em] text-[#28547D]">Missing</p>
            <p className="mt-1 text-3xl font-semibold text-[#28547D]">{data?.missingCount ?? 0}</p>
          </div>
          <div className="rounded-2xl border border-[#0E0E10]/10 bg-white p-4">
            <p className="text-xs font-semibold uppercase tracking-[0.16em] text-[#5C5F66]">Progress</p>
            <p className="mt-1 text-3xl font-semibold text-[#0E0E10]">{progress}%</p>
          </div>
        </div>
      ...
      {CURRENCY_ORDER.map((currency) => {
        ...
        return (
          <article key={currency} className="overflow-hidden rounded-[30px] border border-[#0E0E10]/10 bg-white shadow-[0_18px_54px_rgba(14,14,16,0.07)]">
            <header className="flex flex-col gap-2 border-b border-[#0E0E10]/10 bg-[#F3F7FA] px-5 py-4 sm:flex-row sm:items-center sm:justify-between">
              <div>
                <p className="text-xs font-semibold uppercase tracking-[0.22em] text-[#336699]">{currency}</p>
                <h3 className="mt-1 text-xl font-semibold text-[#0E0E10]">{configured}/6 Stripe prices configured</h3>
              </div>
            ...
            <div className="divide-y divide-[#0E0E10]/8">
              {rows.map((row) => (
                <div key={row.envKey} className="grid gap-3 px-5 py-4 lg:grid-cols-[1.2fr_0.9fr_1.8fr_0.8fr] lg:items-center">
                  <div>
                    <p className="text-sm font-semibold text-[#0E0E10]">{row.planName} · {row.billingInterval}</p>
      ...
      <div className="rounded-[30px] border border-[#0E0E10]/10 bg-[#0E0E10] p-5 text-white shadow-[0_18px_54px_rgba(14,14,16,0.18)] sm:p-6">
        <p className="text-xs font-semibold uppercase tracking-[0.22em] text-[#8FB6D9]">Netlify setup note</p>
