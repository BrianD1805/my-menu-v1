...
export default function AffiliateApplicationForm() {
  ...
  async function submitApplication(event: React.FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      setForm({ applicantName: "", email: "", phone: "", country: "", websiteUrl: "", audienceNotes: "", promotionPlan: "", website: "" });
    } catch (error) {
      setStatus("error");
  ...
  return (
    <form onSubmit={submitApplication} className="overflow-hidden rounded-[32px] border border-[#0E0E10]/10 bg-white shadow-[0_28px_80px_rgba(14,14,16,0.12)]">
      ...
      <div className="grid gap-4 px-5 py-6 sm:px-7 md:grid-cols-2">
        ...
        <label className="block">
          <span className="mb-2 block text-sm font-black text-[#0E0E10]">Country</span>
          <input value={form.country} onChange={(e) => updateField("country", e.target.value)} className="min-h-12 w-full rounded-2xl border border-[#0E0E10]/15 bg-white px-4 py-3 text-sm font-semibold outline-none focus:border-[#FF6A3D] focus:ring-2 focus:ring-[#FF6A3D]/20" />
        </label>
