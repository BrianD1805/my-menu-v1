...
export default function ProductCard({ id, name, description, imageUrl, price, tenantSlug, stockEnabled = false, stockQuantity = 0, lowStockThreshold = 5, variantsEnabled = false, productRequiresVariant = false, variantLabel = "Choose an option", productVariants = [], productType = "standard", customAmountEnabled = false, customAmountLabel = "Amount to pay", customAmountReferenceLabel = "Invoice number", customAmountReferenceRequired = true, customAmountMin = 1, customAmountMax = null, customAmountHelpText = "Enter the amount shown on your invoice.", customAmountDisableRewards = true, customAmountDisableDiscounts = true, preorderEnabled = false, preorderWhenOutOfStock = false, moneySettings, accentColor, primaryColor, themeColors, onAddToCartAnimation, isFavourite = false, favouriteBusy = false, onToggleFavourite, initiallyOpen = false }: Props) {
  ...
  const defaultPopupHeaderText = normalizeThemeColor(themeColors?.defaultPopupHeaderText, "#020617");
  const defaultPopupLabelText = normalizeThemeColor(themeColors?.defaultPopupLabelText, "#B45309");
  ...
  const defaultPopupButtonText = normalizeThemeColor(themeColors?.defaultPopupButtonText, "#FFFFFF");
  const defaultPopupButtonOutline = normalizeThemeColor(themeColors?.defaultPopupButtonOutline, "#E2E8F0");
  const defaultPopupCloseBackground = normalizeThemeColor(themeColors?.defaultPopupCloseBackground, "#FFFFFF");
  const defaultPopupCloseText = normalizeThemeColor(themeColors?.defaultPopupCloseText, "#475569");
          ...
          <div className="flex min-h-full items-center justify-center">
            <div className="flex max-h-[calc(100dvh-150px)] w-full max-w-md flex-col overflow-hidden rounded-[26px] border border-black/5 bg-white shadow-[0_30px_90px_rgba(15,23,42,0.28)]" onClick={(event) => event.stopPropagation()}>
              <div className="sticky top-0 z-10 border-b px-5 pb-5 pt-5" style={{ background: `linear-gradient(135deg, ${defaultPopupHeaderBackground}, ${defaultPopupHeaderBlend}, ${defaultPopupHeaderSoftEnd})`, borderColor: defaultPopupFooterBorder }}>
                ...
                <button type="button" onClick={() => setPreorderConfirmOpen(false)} className="absolute right-4 top-4 inline-flex h-10 w-10 items-center justify-center rounded-full border text-xl shadow-sm" style={{ backgroundColor: defaultPopupCloseBackground, borderColor: defaultPopupButtonOutline, color: defaultPopupCloseText }} aria-label="Close pre-order notice">×</button>
              ...
              <div className="sticky top-0 z-10 border-b border-slate-100 bg-gradient-to-br from-white via-slate-50 to-emerald-50/50 px-4 pb-5 pt-4 sm:px-6 sm:pb-6 sm:pt-5 lg:px-7">
                ...
                <div className="flex items-start justify-between gap-4">
                  <div className="min-w-0">
                    ...
                    <p className="mt-2 text-sm leading-6" style={{ color: defaultPopupBodyText }}>{productRequiresVariant ? "Choose an available option before adding this product to your basket." : "Choose the standard product as shown, or select another available option."}</p>
                  </div>
