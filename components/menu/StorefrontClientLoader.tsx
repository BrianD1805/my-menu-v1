...
const STOREFRONT_CACHE_VERSION = "ver-0-265A";
...
function readCachedPayload(tenantSlug: string): StorefrontPayload | null {
  ...
  try {
    ...
    if (
      !Array.isArray(parsed.payload.products) ||
...
function StorefrontPreparingShell() {
  // Keep this first-render shell completely static. Next renders client components
  // on the server for the initial HTML, so no cached payload, logo colour,
  // localStorage value or dynamic inline style can be used here without risking
  // a hydration mismatch. Store-specific colours are applied only after the
  // payload has loaded and MenuBrowser is rendered.
  return (
    <main
      aria-busy="true"
    ...
    >
      <section className="flex max-w-[320px] flex-col items-center justify-center text-center md:max-w-[620px]">
        ...
        <p className="mt-2 text-sm font-semibold leading-6 text-slate-600">
          Loading the menu, rewards and offers so the store opens smoothly.
        </p>
        <div
  ...
  return (
    <main className="mx-auto min-h-screen max-w-7xl overflow-x-clip bg-[#F8F4F0] px-4 pb-10 pt-4 sm:px-5 lg:px-6">
      <section className="rounded-[32px] border border-red-200 bg-white px-5 py-6 shadow-[0_18px_48px_rgba(15,23,42,0.08)]">
        ...
        <h1 className="mt-2 text-2xl font-black tracking-tight text-[#2B2B2B]">
        ...
        <button
          type="button"
          onClick={onRetry}
          className="mt-5 inline-flex min-h-11 items-center justify-center rounded-2xl bg-[#2B2B2B] px-5 py-3 text-sm font-black text-white shadow-sm"
        >
          Try again
        </button>
      </section>
...
export default function StorefrontClientLoader({
  tenantSlug,
  version,
  initialProductId,
}: {
  ...
  const hideEarlyPreloader = useCallback(() => {
    if (typeof window === "undefined") return;
    const hide = (
      window as Window & { __ORDUVA_HIDE_EARLY_PRELOADER__?: () => void }
    ).__ORDUVA_HIDE_EARLY_PRELOADER__;
    ...
    async function loadStorefront() {
      ...
      try {
        ...
        if (!res.ok) {
          if (!payload)
            setError(
              String(data?.error || "The storefront data could not be loaded."),
            );
          return;
        }
        ...
        if (!payload) {
          setError(
            err instanceof DOMException && err.name === "AbortError"
              ? "The menu request took too long. Please check the connection and try again."
  ...
  const pageBackground = useMemo(
    () =>
      settings?.storefrontTheme?.globalPageBackground ||
  ...
  if (error)
    return (
      <ErrorStorefrontShell
        message={error}
        onRetry={() => setRetryKey((key) => key + 1)}
      />
    );
  if (!payload || !settings)
    return <StorefrontPreparingShell />;

  return (
    <main
      className="mx-auto min-h-screen max-w-7xl overflow-x-clip px-4 pb-10 pt-0 sm:px-5 lg:px-6"
    ...
    >
      <MenuBrowser
        ...
        invoicePaymentsInvoiceEnabled={
          settings.invoicePaymentsInvoiceEnabled !== false
        }
        invoicePaymentsDepositEnabled={
          settings.invoicePaymentsDepositEnabled !== false
        }
