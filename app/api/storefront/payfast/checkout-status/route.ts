...
import { reconcilePayFastIntent } from "@/lib/storefront-payfast";
...
export async function GET(req: Request) {
  try {
    ...
    const transactionReference = first(url.searchParams.get("m_payment_id") || url.searchParams.get("transactionReference"));
    const transactionId = first(url.searchParams.get("pf_payment_id") || url.searchParams.get("transactionId"));

    if (!checkoutId && !transactionReference && !transactionId) {
      return NextResponse.json({ error: "Missing PayFast checkout reference." }, { status: 400 });
    }

    const result = await reconcilePayFastIntent({ checkoutId, transactionReference, transactionId });
    if (!result) return NextResponse.json({ error: "PayFast checkout was not found." }, { status: 404 });
    ...
    console.error("Storefront PayFast checkout status failed", error);
    const message = error instanceof Error ? error.message : "Could not check PayFast payment status.";
