...
function buildSimpleReceiptPdf({ tenantName, currencyCode, order }: { tenantName: string; currencyCode: string; order: any }) {
  const items = Array.isArray(order?.order_items) ? order.order_items : [];
  const receiptRef = receiptNumber(order);
  const total = asMoney(order?.total, currencyCode);
  const subtotal = asMoney(order?.subtotal_total ?? order?.total, currencyCode);
  const lines: string[] = [
    "Receipt",
    tenantName,
    `Receipt: ${receiptRef}`,
    ...
    `Payment: ${order?.payment_method_label || order?.payment_provider || "Order payment"}`,
    order?.payment_reference ? `Reference: ${order.payment_reference}` : "",
    "",
    "Items",
    ...items.slice(0, 18).map((item: any) => `${item?.quantity || 0} x ${item?.product_name || "Item"} - ${asMoney(item?.line_total, currencyCode)}`),
    "",
    `Subtotal: ${subtotal}`,
    Number(order?.reward_discount_amount || 0) > 0 ? `Rewards discount: -${asMoney(order.reward_discount_amount, currencyCode)}` : "",
    Number(order?.discount_amount || 0) > 0 ? `Discount: -${asMoney(order.discount_amount, currencyCode)}` : "",
    `Total paid: ${total}`,
    "",
    `Generated by ${tenantName}. Please quote this receipt if you contact the store.`,
  ].filter(Boolean);

  const content = ["BT", "/F1 20 Tf", "50 790 Td", `(Receipt) Tj`, "/F1 13 Tf", "0 -28 Td"];
  lines.slice(1).forEach((line, index) => {
    const clean = pdfText(line);
    const leading = index === 0 ? 18 : 17;
    content.push(`0 -${leading} Td (${clean}) Tj`);
  });
  content.push("ET");
  ...
  <script>
    async function shareReceipt() {
      ...
      try {
        ...
        alert("Receipt link copied. Use Print / save as PDF to create a PDF file.");
      } catch (error) {
...
export async function GET(req: Request, context: ReceiptParams) {
  ...
  if (requestUrl.searchParams.get("format") === "pdf") {
    const pdf = buildSimpleReceiptPdf({ tenantName, currencyCode, order });
