...
import { CUSTOM_DOMAIN_DNS_TARGET } from "@/lib/custom-domain-addon";
...
async function loadDomains() {
  const { data, error } = await db
    ...
    .select("id, tenant_id, domain_name, normalized_domain, status, billing_status, addon_price_currency, addon_price_monthly, billing_interval, requested_by_email, tenant_notes, owner_notes, dns_target, verification_token, approved_at, activated_at, created_at, updated_at, tenants(name, slug)")
...
export async function GET(req: Request) {
  ...
  try {
    return jsonNoStore({ ok: true, dnsTarget: CUSTOM_DOMAIN_DNS_TARGET, domains: await loadDomains() });
  } catch {
...
export async function PATCH(req: Request) {
  ...
  try {
    ...
    if (Object.prototype.hasOwnProperty.call(body, "stripeSubscriptionItemId")) payload.stripe_subscription_item_id = cleanText(body?.stripeSubscriptionItemId, 120);
    if (Object.prototype.hasOwnProperty.call(body, "stripePriceId")) payload.stripe_price_id = cleanText(body?.stripePriceId, 120);
