...
export default async function OzowPaymentErrorPage({ searchParams }: { searchParams: Promise<Record<string, string | string[] | undefined>> }) {
  const params = await searchParams;
  const checkoutId = first(params.checkout_id).trim();
  let tenantSlug = "";

  if (checkoutId) {
    const { data } = await db.from("storefront_payment_intents").select("id,tenant_id,order_payload").eq("id", checkoutId).eq("provider", "ozow").maybeSingle();
    tenantSlug = readTenantSlug((data as Record<string, unknown> | null)?.order_payload);
    await db.from("storefront_payment_intents").update({ status: "failed", updated_at: new Date().toISOString() }).eq("id", checkoutId).eq("provider", "ozow").is("order_id", null);
  }
  ...
  return (
    <main className="min-h-screen bg-[#F8FAF7] px-4 py-8 text-slate-900">
      <section className="mx-auto max-w-xl rounded-[32px] border border-red-100 bg-white p-6 text-center sm:p-8">
        ...
        <p className="mt-5 text-xs font-black uppercase tracking-[0.28em] text-red-700">Ozow payment</p>
        <h1 className="mt-3 text-3xl font-black tracking-tight text-slate-950">Payment was not completed</h1>
        <p className="mt-3 text-sm leading-6 text-slate-600">Ozow reported an error while processing the payment. Your basket has not been sent to the store yet.</p>
