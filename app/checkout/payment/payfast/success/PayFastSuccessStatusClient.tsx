...
export default function PayFastSuccessStatusClient({ checkoutId }: { checkoutId: string }) {
  ...
  useEffect(() => {
    ...
    async function loadStatus(attempt = 0) {
      ...
      try {
        const response = await fetch(`/api/storefront/payfast/checkout-status?${params.toString()}`, { cache: "no-store" });
        ...
        if (!cancelled) setStatus({ checkoutId, transactionReference: null, transactionId: null, status: "checking", paid: false, orderId: null, checkoutUrl: "/checkout", error: "Still checking PayFast payment status." });
      } finally {
  ...
  return (
    <main className="min-h-screen bg-[#F8FAF7] px-4 py-8 text-slate-900">
      <section className="mx-auto max-w-xl rounded-[32px] border border-emerald-100 bg-white p-6 text-center sm:p-8">
        ...
        <p className="mt-5 text-xs font-black uppercase tracking-[0.28em] text-emerald-700">PayFast payment</p>
        <h1 className="mt-3 text-3xl font-black tracking-tight text-slate-950">
          {paid ? "Payment received" : loading ? "Confirming payment" : "Payment is being checked"}
        </h1>
        <p className="mt-3 text-sm leading-6 text-slate-600">
          {paid
            ? "Your PayFast payment has been confirmed and the order has been sent to the store."
            : "PayFast has returned you to Orduva. We are checking the payment confirmation before creating the order. Please do not pay again."}
        </p>
