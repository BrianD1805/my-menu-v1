...
export default async function PayFastPaymentCancelPage({ searchParams }: { searchParams: Promise<Record<string, string | string[] | undefined>> }) {
  ...
  if (checkoutId) {
    ...
    await db.from("storefront_payment_intents").update({ status: "cancelled", updated_at: new Date().toISOString() }).eq("id", checkoutId).eq("provider", "payfast").is("order_id", null);
  }
  ...
  return (
    <main className="min-h-screen bg-[#F8FAF7] px-4 py-8 text-slate-900">
      <section className="mx-auto max-w-xl rounded-[32px] border border-amber-100 bg-white p-6 text-center sm:p-8">
        ...
        <p className="mt-5 text-xs font-black uppercase tracking-[0.28em] text-amber-700">PayFast payment</p>
