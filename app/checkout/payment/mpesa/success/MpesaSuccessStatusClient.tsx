...
export default function MpesaSuccessStatusClient({ checkoutId, orderTrackingId, merchantReference }: { checkoutId: string; orderTrackingId: string; merchantReference: string }) {
  ...
  useEffect(() => {
    ...
    async function loadStatus(attempt = 0) {
      ...
      try {
        const response = await fetch(`/api/storefront/mpesa/checkout-status?${params.toString()}`, { cache: "no-store" });
        ...
        if (!cancelled && !data?.paid && attempt < 10) {
          timer = window.setTimeout(() => void loadStatus(attempt + 1), attempt < 4 ? 2000 : 4500);
        }
      } catch {
        if (!cancelled) setStatus({ checkoutId, orderTrackingId, merchantReference, status: "checking", paid: false, orderId: null, checkoutUrl: "/checkout", error: "Still checking M-Pesa payment status." });
      } finally {
  ...
  }, [checkoutId, orderTrackingId, merchantReference]);
  ...
  return (
    <main className="min-h-screen bg-[#F8FAF7] px-4 py-8 text-slate-900">
      <section className="mx-auto max-w-xl rounded-[32px] border border-emerald-100 bg-white p-6 text-center sm:p-8">
        ...
        <p className="mt-5 text-xs font-black uppercase tracking-[0.28em] text-emerald-700">M-Pesa payment</p>
        ...
        <p className="mt-3 text-sm leading-6 text-slate-600">
          {paid
            ...
            : "We are checking Pesapal for the final M-Pesa confirmation. Mobile money payments can sometimes remain pending for a few moments."}
        </p>
