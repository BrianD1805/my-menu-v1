...
export default async function YocoCancelPage({ searchParams }: { searchParams: Promise<Record<string, string | string[] | undefined>> }) {
  ...
  return (
    <main className="min-h-screen bg-[#F8FAF7] px-4 py-8 text-slate-900">
      <section className="mx-auto max-w-xl rounded-[32px] border border-amber-100 bg-white p-6 text-center sm:p-8">
        ...
        <p className="mt-5 text-xs font-black uppercase tracking-[0.28em] text-amber-700">Yoco checkout cancelled</p>
        ...
        <div className="mt-6 flex flex-col gap-3 sm:flex-row">
          <a href="/checkout" className="inline-flex min-h-12 flex-1 items-center justify-center rounded-2xl bg-emerald-700 px-5 py-3 text-sm font-black text-white transition hover:bg-emerald-800">Return to checkout</a>
          <a href="/" className="inline-flex min-h-12 flex-1 items-center justify-center rounded-2xl border border-slate-200 bg-white px-5 py-3 text-sm font-black text-slate-800 transition hover:bg-slate-50">Back to store</a>
        </div>
      </section>
    </main>
  );
