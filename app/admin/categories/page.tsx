...
export default async function AdminCategoriesPage() {
  ...
  const { data: products } = await db
    .from("products")
    .select("id, category_id, secondary_category_id")
    .eq("tenant_id", tenant.id);
  ...
  for (const product of products || []) {
    ...
    if (product.secondary_category_id) {
      productCountByCategory.set(
        ...
        (productCountByCategory.get(product.secondary_category_id) || 0) + 1,
      );
    }
  }
  ...
  return (
    ...
    >
      <div className="mb-6 rounded-[24px] border border-sky-100 bg-sky-50 p-4 text-sm text-sky-900">
        You can add and reorder categories here. Deleting a category is blocked
        if products still belong to it.
      </div>
