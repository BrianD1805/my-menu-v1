...
export default async function StripeBillingSuccessPage({ searchParams }: Props) {
  ...
  const currency = safeText(params.currency, "");
      ...
      <div className="mx-auto flex min-h-[calc(100vh-4rem)] max-w-3xl items-center justify-center">
        <section className="w-full overflow-hidden rounded-[34px] border border-[#E8D8C8] bg-white shadow-[0_30px_90px_rgba(81,55,45,0.14)]">
          ...
          <div className="px-6 py-8 text-center sm:px-10 sm:py-10">
            ...
            <h1 className="mt-5 text-3xl font-black tracking-tight text-[#0E0E10] sm:text-4xl">Thank you — your plan is being activated.</h1>
            <p className="mx-auto mt-4 max-w-xl text-sm leading-7 text-[#5C5F66] sm:text-base">
              Stripe has returned successfully for <span className="font-black text-[#0E0E10]">{tenant}</span>. Your payment has been received by Stripe. Orduva will activate your plan automatically as soon as Stripe confirms the subscription.
            </p>
            <div className="mx-auto mt-6 grid max-w-xl gap-3 rounded-[26px] border border-[#E8D8C8] bg-[#FFF8EF] p-4 text-left text-sm sm:grid-cols-3">
              <div><p className="font-black text-[#B74A16]">Plan</p><p className="mt-1 capitalize text-[#0E0E10]">{plan}</p></div>
              <div><p className="font-black text-[#B74A16]">Billing</p><p className="mt-1 capitalize text-[#0E0E10]">{billing}</p></div>
              <div><p className="font-black text-[#B74A16]">Currency</p><p className="mt-1 text-[#0E0E10]">{currency || "Selected"}</p></div>
            </div>
